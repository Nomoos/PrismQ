...
def check_completion_status(filepath: Path) -> Dict[str, any]:
    ...
    has_wip_marker = bool(re.search(r'(🚧|IN PROGRESS|WIP|\d+%\s+complete)', content, re.IGNORECASE))
    ...
    return {
        ...
        'appears_complete': (
            has_completion_marker or 
            (total_items > 0 and completion_percentage == 100)
        )
    }
...
def generate_recommendations(metadata: IssueMetadata, completion: Dict) -> List[str]:
    ...
    if metadata.status and metadata.status.lower() in ['done', 'complete', 'completed']:
        recommendations.append("⚠️  Status is 'Done' but issue is in WIP folder - SHOULD MOVE TO done/")
    elif metadata.status and metadata.status.lower() == 'new':
        recommendations.append("⚠️  Status is 'New' but issue is in WIP folder - update status or move to new/")
    ...
    if completion['has_checklist'] and completion['completion_percentage'] == 100:
        recommendations.append(f"✅ All checklist items completed ({completion['completed_items']}/{completion['total_items']}) - consider moving to done/")
    elif completion['has_checklist'] and completion['completion_percentage'] > 0:
        recommendations.append(f"🚧 Checklist progress: {completion['completed_items']}/{completion['total_items']} ({completion['completion_percentage']:.1f}%) - actively in progress")
...
def main():
    ...
    for issue_file in sorted(issue_files):
        ...
        if completion['has_checklist']:
            print(f"  Checklist: {completion['completed_items']}/{completion['total_items']} items completed ({completion['completion_percentage']:.1f}%)")
        ...
        # Categorize issues
        if completion['appears_complete'] or (metadata.status and metadata.status.lower() in ['done', 'complete', 'completed']):
            issues_to_move.append(metadata.filename)
        elif completion['has_wip_marker'] or (completion['has_checklist'] and completion['incomplete_items'] > 0):
