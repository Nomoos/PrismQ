...
class BaseYouTubeWorker(BaseVideoSourceWorker, ABC):
    ...
    def get_quota_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'percentage_used': (usage['total_used'] / usage['daily_limit']) * 100
                if usage['daily_limit'] > 0 else 0.0
        }
    ...
    def fetch_youtube_videos_batch(
        self,
        video_ids: list
    ) -> list:
    ...
    def handle_youtube_error(self, error: HttpError) -> TaskResult:
        ...
        return TaskResult(
            success=False,
            error=f"YouTube API error: {error_str[:200]}"
        )
