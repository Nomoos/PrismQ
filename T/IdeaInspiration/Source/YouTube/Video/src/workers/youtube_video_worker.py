...
class YouTubeVideoWorker(BaseWorker):
    ...
    def __init__(
        ...
        **kwargs
        ...
        try:
            ...
            logger.warning(f"SubtitleExtractor initialization failed: {e}. Subtitles will not be available.")
    ...
    def _process_single_video(self, task: Task) -> TaskResult:
        ...
        if not self.youtube_client.can_execute_operation('videos.list'):
            ...
            return TaskResult(
                success=False,
                error=f"YouTube API quota exceeded. Remaining: {remaining} units"
            )
        ...
        try:
            ...
            return TaskResult(
                success=False,
                error=f"YouTube API error for video {video_id}: {e}"
            )
    ...
    def _process_search(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            return TaskResult(
                success=False,
                error=f"YouTube API error during search: {e}"
            )
    ...
    def _video_to_idea(self, video: Dict[str, Any]) -> IdeaInspiration:
        ...
        if self.subtitle_extractor:
            try:
                ...
                if subtitle_text:
                    logger.info(f"Successfully extracted {len(subtitle_text)} characters of subtitles")
