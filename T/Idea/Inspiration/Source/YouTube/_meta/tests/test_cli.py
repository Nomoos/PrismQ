        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                with patch("src.cli.YouTubePlugin") as mock_plugin:
                    ...
                    result = runner.invoke(cli.scrape, ["--env-file", temp_env, "--no-interactive"])
...
class TestScrapeChannelCommand:
            ...
            with patch("src.cli.Database") as mock_db:
                with patch("src.cli.YouTubeChannelPlugin") as mock_plugin:
                    ...
                    result = runner.invoke(
                        cli.scrape_channel, ["--channel", "@testchannel", "--no-interactive"]
                    )
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                with patch("src.cli.YouTubeChannelPlugin") as mock_plugin:
                    ...
                    result = runner.invoke(
                        cli.scrape_channel,
                        ["--channel", "@testchannel", "--top", "20", "--no-interactive"],
                    )
    ...
    def test_scrape_channel_handles_ytdlp_error(self, runner):
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                with patch(
                    "src.cli.YouTubeChannelPlugin",
                    side_effect=ValueError("yt-dlp is not installed"),
                ):
                    mock_config.return_value.database_path = "test.db"

                    result = runner.invoke(
                        cli.scrape_channel, ["--channel", "@test", "--no-interactive"]
                    )
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                with patch("src.cli.YouTubeTrendingPlugin") as mock_plugin:
                    ...
                    result = runner.invoke(cli.scrape_trending, ["--top", "15", "--no-interactive"])
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                with patch("src.cli.YouTubeTrendingPlugin") as mock_plugin:
                    ...
                    result = runner.invoke(
                        cli.scrape_keyword, ["--keyword", "test keyword", "--no-interactive"]
                    )
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                with patch("src.cli.YouTubeTrendingPlugin") as mock_plugin:
                    ...
                    result = runner.invoke(
                        cli.scrape_keyword, ["--keyword", "test", "--top", "25", "--no-interactive"]
                    )
    ...
    def test_list_with_source_filter(self, runner):
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.cli.Database") as mock_db:
                ...
                result = runner.invoke(cli.list, ["--source", "youtube", "--no-interactive"])
        ...
        with patch("src.cli.Config") as mock_config:
            with patch("src.core.db_utils.init_database"):
                with patch("src.core.db_utils.get_unprocessed_records") as mock_get:
                    ...
                    result = runner.invoke(cli.process, ["--limit", "5", "--no-interactive"])
                    ...
                    with patch("src.core.db_utils.get_unprocessed_records") as mock_get:
                        with patch("src.core.db_utils.mark_as_processed"):
                            with patch("src.cli.IdeaProcessor.process") as mock_process:
                                ...
                                result = runner.invoke(
                                    cli.process, ["--output", str(output_file), "--no-interactive"]
                                )
...
class TestClearCommand:
    ...
    def test_clear_database(self, runner, temp_db):
        ...
        with patch("src.cli.Config") as mock_config:
            ...
            result = runner.invoke(
                cli.clear, ["--no-interactive", "--yes"], input="y\n"  # Auto-confirm
            )
    ...
    def test_clear_nonexistent_database(self, runner):
        ...
        with patch("src.cli.Config") as mock_config:
            ...
            result = runner.invoke(cli.clear, ["--no-interactive", "--yes"], input="y\n")
