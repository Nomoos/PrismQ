...
def register_default_schemas():
    ...
    channel_schema = TaskParameterSchema(
        ...
        examples=[
            ...
            # With age filter for recent content only
            {'channel_url': 'https://youtube.com/@channel', 'top_n': 100, 'max_age_days': 30},
            # Minimal scrape for quick testing
            {'channel_url': 'https://youtube.com/@testchannel', 'top_n': 10},
            # Large batch with age restriction
            {'channel_url': 'https://youtube.com/@popularchannel', 'top_n': 200, 'max_age_days': 90},
            # Recent videos only (1 week)
            {'channel_url': 'https://youtube.com/@newschannel', 'top_n': 50, 'max_age_days': 7},
            # Maximum results without age filter
            {'channel_url': 'https://youtube.com/@educhannel', 'top_n': 500}
        ]
    )
    ...
    trending_schema = TaskParameterSchema(
        ...
        examples=[
            # Default US trending, all categories
            {'country': 'US', 'top_n': 50},
            # UK music trending with more results
            {'country': 'GB', 'category': 'music', 'top_n': 100},
            # Japanese trending, all categories
            {'country': 'JP', 'top_n': 75},
            # Gaming trending in Canada
            {'country': 'CA', 'category': 'gaming', 'top_n': 60},
            # News category in Germany
            {'country': 'DE', 'category': 'news', 'top_n': 40},
            # Movies in Australian region, max results
            {'country': 'AU', 'category': 'movies', 'top_n': 200},
            # Quick overview of all categories
            {'country': 'FR', 'category': 'all', 'top_n': 20}
        ]
    )
    ...
    keyword_schema = TaskParameterSchema(
        ...
        examples=[
            # Basic search with high result count
            {'query': 'python tutorial', 'top_n': 100},
            # Recent machine learning content sorted by views
            {'query': 'machine learning', 'date_filter': 'month', 'sort_by': 'views'},
            # Today's content sorted by date
            {'query': 'breaking news', 'date_filter': 'day', 'sort_by': 'date', 'top_n': 50},
            # Short videos about quick tips
            {'query': 'coding tips', 'duration_filter': 'short', 'top_n': 75},
            # Long-form educational content from past year
            {'query': 'data science course', 'duration_filter': 'long', 'date_filter': 'year', 'top_n': 150},
            # Recent highly-rated content
            {'query': 'best practices 2024', 'sort_by': 'rating', 'date_filter': 'week', 'top_n': 60},
            # Comprehensive search with all filters
            {'query': 'ai tools', 'date_filter': 'month', 'duration_filter': 'medium', 'sort_by': 'views', 'top_n': 200},
            # Quick relevant search
            {'query': 'productivity hacks', 'sort_by': 'relevance', 'top_n': 30}
        ]
    )
...
def test_default_application():
    ...
    assert prepared['channel_url'] == 'https://youtube.com/@test'
```
...
def test_parameter_edge_cases():
    ...
    schema = registry.get_schema('channel_scrape')
    is_valid, errors = schema.validate_parameters({
        'channel_url': 'https://youtube.com/@test',
        'top_n': 1  # Minimum value
    })
    assert is_valid
    ...
    assert not is_valid
    assert 'must be >=' in str(errors)
    
    # Test above maximum
    is_valid, errors = schema.validate_parameters({
        'channel_url': 'https://youtube.com/@test',
        'top_n': 501  # Above maximum
    })
    assert not is_valid
    assert 'must be <=' in str(errors)
...
def test_enum_validation():
    ...
    schema = ParameterRegistry.get_instance().get_schema('keyword_search')
    ...
    assert not is_valid
    assert 'must be one of' in str(errors)
...
def test_pattern_validation():
    ...
    valid_urls = [
        ...
        'https://www.youtube.com/@test123',
        'http://youtube.com/@test_channel'
    ]
    ...
    # Invalid URL pattern
    is_valid, errors = schema.validate_parameters({
        'channel_url': 'not-a-valid-url',
        'top_n': 50
    })
    assert not is_valid
    assert 'pattern' in str(errors).lower()
```
...
def test_schema_registration():
    ...
    schema_types = [s.task_type for s in schemas]
    
    assert 'channel_scrape' in schema_types
    assert 'trending_scrape' in schema_types
    assert 'keyword_search' in schema_types
    
    # Test individual schema retrieval
    channel_schema = registry.get_schema('channel_scrape')
    assert channel_schema is not None
    assert channel_schema.task_type == 'channel_scrape'
    assert len(channel_schema.parameters) > 0


def test_validate_and_prepare():
    """Test combined validation and preparation."""
    registry = ParameterRegistry.get_instance()
    
    # Valid parameters without optional fields
    is_valid, prepared, errors = registry.validate_and_prepare(
        'channel_scrape',
        {'channel_url': 'https://youtube.com/@test'}
    )
    ...
    assert errors == []
    assert prepared['channel_url'] == 'https://youtube.com/@test'
    assert prepared['top_n'] == 10  # Default applied
    
    # Invalid parameters
    is_valid, prepared, errors = registry.validate_and_prepare(
        'channel_scrape',
        {'top_n': 50}  # Missing required channel_url
    )
...
def test_unknown_task_type():
    ...
    assert not is_valid
    assert 'Unknown task type' in str(errors)
...
def test_multiple_validation_errors():
    ...
    # Multiple errors: missing required, out of range
    is_valid, errors = registry.validate_and_prepare(
        'keyword_search',
        {
            # Missing required 'query'
            'top_n': 1000,  # Above maximum
            'sort_by': 'invalid'  # Invalid enum
        }
    )
    
    assert not is_valid
    assert len(errors[2]) >= 2  # Multiple errors
```
...
def test_validation_performance():
    ...
    # Prepare 1000 validation attempts
    params = {
        'channel_url': 'https://youtube.com/@test',
        'top_n': 50
    }
    ...
    start = time.time()
    for _ in range(1000):
        registry.validate_and_prepare('channel_scrape', params)
    elapsed = time.time() - start
    
    # Should complete 1000 validations in less than 1 second
    assert elapsed < 1.0, f"Validation too slow: {elapsed}s for 1000 operations"
...
def test_concurrent_access():
    ...
    def validate_task():
        for _ in range(100):
            is_valid, _, errors = registry.validate_and_prepare(
                'channel_scrape',
                {'channel_url': 'https://youtube.com/@test'}
            )
    ...
    threads = [threading.Thread(target=validate_task) for _ in range(10)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
