...
class Database:
    ...
    def _confirm_database_creation(self) -> bool:
        ...
        try:
            response = (
                input(f"Database '{self.db_path}' does not exist. Create it? (y/n): ")
                .strip()
                .lower()
            )
    ...
    def insert_idea(
        ...
        return db_utils.insert_idea(
            self.database_url, source, source_id, title, description, tags, score, score_dictionary
        )
    ...
    def get_idea(self, source: str, source_id: str) -> Optional[Dict[str, Any]]:
        ...
        with db_utils.get_connection(self.database_url) as conn:
            result = conn.execute(
                text(
                    "SELECT * FROM YouTubeShortsSource WHERE source = :source AND source_id = :source_id"
                ),
