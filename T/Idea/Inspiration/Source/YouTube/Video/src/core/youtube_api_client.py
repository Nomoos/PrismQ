...
from .youtube_quota_manager import QuotaExceededException, YouTubeQuotaManager
...
class YouTubeAPIClient:
    ...
    def __init__(
        ...
        self.quota_manager = YouTubeQuotaManager(
            storage_path=quota_storage_path, daily_limit=daily_quota_limit
        )

        logger.info("YouTubeAPIClient initialized with quota management (JSON storage)")

    def _execute_with_quota(self, operation: str, request_callable) -> Optional[Dict[str, Any]]:
    ...
    def get_video_details(
        self, video_id: str, parts: Optional[List[str]] = None
        ...
        def make_request():
            return self.youtube.videos().list(part=parts_str, id=video_id)
    ...
    def get_videos_batch(
        self, video_ids: List[str], parts: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        ...
        def make_request():
            return self.youtube.videos().list(part=parts_str, id=ids_str)
    ...
    def get_channel_details(
        self, channel_id: str, parts: Optional[List[str]] = None
    ) -> Optional[Dict[str, Any]]:
        ...
        def make_request():
            return self.youtube.channels().list(part=parts_str, id=channel_id)
    ...
    def get_comment_threads(self, video_id: str, max_results: int = 20) -> List[Dict[str, Any]]:
