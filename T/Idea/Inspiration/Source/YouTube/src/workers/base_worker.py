...
class BaseWorker(ABC):
    ...
    - Queue implementation (TaskManager API handles this)
    ...
    def __init__(
        ...
        if not _taskmanager_available:
            raise ImportError(
                "TaskManager module is required. " "Install with: pip install -e Source/TaskManager"
            )
        if not task_type_ids:
            raise ValueError(
                "task_type_ids is required. " "Register task types and provide their IDs."
            )
        ...
        try:
            ...
            for task_type_id in self.task_type_ids:
                try:
                    ...
                    logger.error(
                        f"Error claiming task from TaskManager API (type_id {task_type_id}): {e}"
                    )
                    continue
    ...
    def _update_task_manager(self, task: Task, result: TaskResult) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Task {task.id} status reported to TaskManager API " f"(success: {result.success})"
            )
    ...
    def _increase_backoff(self):
        ...
        self._current_backoff = min(
            self._current_backoff * self.backoff_multiplier, self.max_backoff
        )
        logger.debug(
