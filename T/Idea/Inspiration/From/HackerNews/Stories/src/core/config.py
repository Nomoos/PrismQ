"""Configuration management for PrismQ.IdeaInspiration.Froms.Content.Forums.HackerNews."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        self.hn_request_timeout = int(
            self._get_or_prompt(
                "HN_REQUEST_TIMEOUT", "Request timeout in seconds", "10", required=False
            )
        )

        # Scraping configuration
        hn_frontpage_max_posts = self._get_or_prompt(
            "HN_FRONTPAGE_MAX_POSTS", "Maximum posts to scrape from frontpage", "10", required=False
        )
        ...
        hn_new_max_posts = self._get_or_prompt(
            "HN_NEW_MAX_POSTS", "Maximum posts to scrape from new stories", "10", required=False
        )
        self.hn_new_max_posts = int(hn_new_max_posts) if hn_new_max_posts else 10

        hn_best_max_posts = self._get_or_prompt(
            "HN_BEST_MAX_POSTS", "Maximum posts to scrape from best stories", "10", required=False
        )
