...
def insert_idea(
    ...
    with get_connection(database_url) as conn:
        ...
        result = conn.execute(
            text(
                "SELECT id FROM HackerNewsSource WHERE source = :source AND source_id = :source_id"
            ),
...
def count_by_source(database_url: str, source: str) -> int:
    ...
    with get_connection(database_url) as conn:
        result = conn.execute(
            text("SELECT COUNT(*) FROM HackerNewsSource WHERE source = :source"), {"source": source}
        )
        return result.scalar()
...
def mark_as_processed(database_url: str, record_id: int):
    ...
    with get_connection(database_url) as conn:
        conn.execute(
            text(
                "UPDATE HackerNewsSource SET processed = 1, updated_at = :updated_at WHERE id = :id"
            ),
