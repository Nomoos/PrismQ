...
WORKFLOW_STATE_ORDER = {
    ...
    "PrismQ.T.Review.Title.From.Content.Idea": 4,
    "PrismQ.T.Review.Content.From.Title.Idea": 5,
    "PrismQ.T.Review.Title.From.Content": 6,
    "PrismQ.T.Title.From.Content.Review.Title": 7,
    "PrismQ.T.Content.From.Title.Review.Content": 8,
    "PrismQ.T.Review.Content.From.Title": 9,
...
def get_strategy(name: str) -> ClaimingStrategy:
    ...
    if not strategy:
        raise ValueError(
            f"Unknown strategy '{name}'. " f"Available: {', '.join(_STRATEGIES.keys())}"
        )
    return strategy
