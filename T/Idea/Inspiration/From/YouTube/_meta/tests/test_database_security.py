...
def test_order_by_validation():
    ...
    # Insert test data
    db.insert_idea(source="test", source_id="1", title="Test Idea", score=10.0)

    # Valid order_by values should work
    valid_values = ["score DESC", "score ASC", "title", "created_at DESC", "id ASC"]
...
def test_order_by_edge_cases():
    ...
    # Insert test data
    for i in range(3):
        db.insert_idea(source="test", source_id=str(i), title=f"Test Idea {i}", score=float(i))
