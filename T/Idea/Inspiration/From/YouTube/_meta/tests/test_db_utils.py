...
class TestInsertIdea:
    ...
    def test_insert_with_optional_fields(self, temp_db_url):
        ...
        result = db_utils.insert_idea(
            database_url=temp_db_url, source="youtube", source_id="test456", title="Minimal Video"
        )
        ...
        with db_utils.get_connection(temp_db_url) as conn:
            result = conn.execute(
                db_utils.text("SELECT * FROM YouTubeShortsSource WHERE source_id = :sid"),
                {"sid": "test456"},
            )
            row = result.fetchone()
    ...
    def test_insert_sets_timestamps(self, temp_db_url):
        ...
        db_utils.insert_idea(
            database_url=temp_db_url, source="youtube", source_id="test789", title="Test Video"
        )
...
class TestGetUnprocessedRecords:
    ...
    def test_excludes_processed_records(self, temp_db_url):
        ...
        # Insert unprocessed record
        db_utils.insert_idea(
            database_url=temp_db_url, source="youtube", source_id="video1", title="Video 1"
        )

        # Insert and mark as processed
        db_utils.insert_idea(
            database_url=temp_db_url, source="youtube", source_id="video2", title="Video 2"
        )
...
class TestMarkAsProcessed:
    ...
    def test_mark_record_as_processed(self, temp_db_url):
        ...
        with db_utils.get_connection(temp_db_url) as conn:
            result = conn.execute(
                db_utils.text("SELECT id FROM YouTubeShortsSource WHERE source_id = :sid"),
                {"sid": "video1"},
            )
        ...
        with db_utils.get_connection(temp_db_url) as conn:
            result = conn.execute(
                db_utils.text("SELECT processed FROM YouTubeShortsSource WHERE id = :id"),
                {"id": record_id},
            )
    ...
    def test_mark_as_processed_updates_timestamp(self, temp_db_url):
        ...
        with db_utils.get_connection(temp_db_url) as conn:
            result = conn.execute(
                db_utils.text("SELECT updated_at FROM YouTubeShortsSource WHERE id = :id"),
                {"id": record_id},
            )
...
class TestGetAllIdeas:
    ...
    def test_order_by_sql_injection_protection(self, temp_db_url):
        ...
        ideas = db_utils.get_all_ideas(
            temp_db_url, order_by="score; DROP TABLE YouTubeShortsSource;"
        )
