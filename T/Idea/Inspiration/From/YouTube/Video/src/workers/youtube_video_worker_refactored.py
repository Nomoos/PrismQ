    ...
    **Example Usage**:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"YouTubeVideoWorker {worker_id} initialized " f"(idea_db: {self.idea_db_path})"
        )
    ...
    def _process_single_video(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            if not self.validate_video_metadata(video_data):
                return TaskResult(success=False, error=f"Invalid video metadata for {video_id}")
            ...
            if not idea:
                return TaskResult(
                    success=False, error=f"Failed to create IdeaInspiration for {video_id}"
                )
            ...
            return TaskResult(success=False, error=f"Error processing video {video_id}: {str(e)}")
    ...
    def _process_search(self, task: Task) -> TaskResult:
        ...
        try:
            ...
            for youtube_item in videos:
                try:
                    ...
                    if idea:
                        idea_id = self.idea_db.insert(idea)
                        ideas_saved.append(
                            {"video_id": video_data["id"], "idea_id": idea_id, "title": idea.title}
                        )
                except Exception as e:
            ...
            return TaskResult(success=False, error=f"Error processing search: {str(e)}")
