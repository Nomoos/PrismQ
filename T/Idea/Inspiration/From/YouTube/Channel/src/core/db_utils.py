...
def insert_idea(
    ...
    with get_connection(database_url) as conn:
        ...
        result = conn.execute(
            text(
                "SELECT id FROM YouTubeShortsSource WHERE source = :source AND source_id = :source_id"
            ),
...
def mark_as_processed(database_url: str, record_id: int):
    ...
    with get_connection(database_url) as conn:
        conn.execute(
            text(
                "UPDATE YouTubeShortsSource SET processed = 1, updated_at = :updated_at WHERE id = :id"
            ),
