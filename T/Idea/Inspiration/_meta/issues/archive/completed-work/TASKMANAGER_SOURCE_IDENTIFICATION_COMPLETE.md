...
class TaskTypeRegistry:
    ...
    def build_task_type_name(cls, media_type, platform, source_type):
        """Build task type name: PrismQ.T.Idea.Inspiration.From.{MediaType}.{Platform}.{SourceType}"""
        return f"PrismQ.T.Idea.Inspiration.From.{media_type}.{platform}.{source_type}"
```

### Convenience Functions

```python
task_type_youtube_video()      # "PrismQ.T.Idea.Inspiration.From.Video.YouTube.Video"
task_type_youtube_channel()    # "PrismQ.T.Idea.Inspiration.From.Video.YouTube.Channel"
task_type_reddit_posts()       # "PrismQ.T.Idea.Inspiration.From.Text.Reddit.Posts"
# ... and more
```
...
client.register_task_type(
    name="PrismQ.T.Idea.Inspiration.From.Video.YouTube.Video",
    version="1.0.0",
    param_schema={
        "type": "object",
        "properties": {
...
# 2. Create task
task = client.create_task(
    task_type="PrismQ.T.Idea.Inspiration.From.Video.YouTube.Video",
    params={"video_id": "abc123"}
)

# 3. Worker claims and processes
task_type_info = client.get_task_type(
    "PrismQ.T.Idea.Inspiration.From.Video.YouTube.Video"
)
task = client.claim_task(
