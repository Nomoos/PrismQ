...
class CoverageAnalyzer:
    ...
    def run_module_tests(self, module_path: Path) -> Optional[ModuleCoverage]:
        ...
        pytest_args.extend(
            ["--cov-report=json:coverage.json", "--cov-report=term", "-v", "--tb=no", "-q"]
        )
        ...
        try:
            ...
            print(
                f"📊 Coverage: {coverage.coverage_percentage:.1f}% ({coverage.covered_statements}/{coverage.total_statements} statements)"
            )
        ...
        for module in self.modules:
            if module.partially_covered_files:
                ...
                for file_info in sorted(
                    module.partially_covered_files, key=lambda x: x["coverage"]
                ):
                    report.append(
