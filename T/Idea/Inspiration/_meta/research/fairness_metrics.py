...
def analyze_starvation_risk(
    wait_times_by_priority: Dict[int, List[float]], starvation_threshold_seconds: float = 300.0
...
def calculate_probability_distribution(selections_by_priority: Dict[int, int]) -> Dict[int, float]:
    """
    ...
    return {
        priority: count / total_selections for priority, count in selections_by_priority.items()
    }


def calculate_expected_weighted_random_probability(priorities: List[int]) -> Dict[int, float]:
    """
