...
Examples:
  ...
  python create_variants.py --list
    ...
    parser.add_argument(
        "input", nargs="?", help="Input text (title, description, keyword, story snippet, or JSON)"
    )
    parser.add_argument("--file", "-f", help="Read input from file")
    parser.add_argument(
        "--variant",
        "-v",
        help="Specific variant type to create (default: creates all simple variants)",
    )
    parser.add_argument(
        "--count", "-c", type=int, default=10, help="Number of variants to create (default: 10)"
    )
    parser.add_argument("--all", "-a", action="store_true", help="Create all 11 variant types")
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available variant templates"
    )
