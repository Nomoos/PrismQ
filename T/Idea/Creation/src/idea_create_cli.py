...
def validate_idea(idea: Idea) -> List[str]:
    ...
    if not isinstance(idea.concept, str):
        errors.append(f"Invalid type for 'concept': expected str, got {type(idea.concept).__name__}")
    
    if not isinstance(idea.keywords, list):
        errors.append(f"Invalid type for 'keywords': expected list, got {type(idea.keywords).__name__}")
    ...
    if not isinstance(idea.target_platforms, list):
        errors.append(f"Invalid type for 'target_platforms': expected list, got {type(idea.target_platforms).__name__}")
    
    if not isinstance(idea.target_formats, list):
        errors.append(f"Invalid type for 'target_formats': expected list, got {type(idea.target_formats).__name__}")
    
    # Validate status is valid enum
    if not isinstance(idea.status, IdeaStatus):
        errors.append(f"Invalid type for 'status': expected IdeaStatus, got {type(idea.status).__name__}")
    
    # Validate genre is valid enum
    if not isinstance(idea.genre, ContentGenre):
        errors.append(f"Invalid type for 'genre': expected ContentGenre, got {type(idea.genre).__name__}")
...
def format_idea_row(idea: Idea, index: int, verbose: bool = False) -> str:
    ...
    if verbose and idea.synopsis:
        lines.append(f"📖 Synopsis: {idea.synopsis[:200]}..." if len(idea.synopsis) > 200 else f"📖 Synopsis: {idea.synopsis}")
