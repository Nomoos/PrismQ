...
from src.batch_processor import BatchProcessor, BatchConfig, ProcessingMode
...
async def example_small_batch():
    ...
    # Create sample ideas
    ideas = [
        {"id": f"idea-{i}", "text": f"AI in healthcare topic {i}"}
        for i in range(1, 11)
    ]
    ...
    report = await processor.process_batch(
        ideas=ideas,
        process_func=simple_idea_processor,
        batch_id='example-small-batch'
    )
...
async def example_medium_batch():
    ...
    ideas = [
        {
            "id": f"idea-{i}", 
            "text": f"Climate change solution {i}",
...
async def example_sequential_processing():
    ...
    ideas = [
        {"id": f"idea-{i}", "text": f"Tech innovation {i}"}
        for i in range(1, 11)
    ]
...
async def example_with_failures():
    ...
    ideas = [
        {"id": f"idea-{i}", "text": f"Test idea {i}"}
        for i in range(1, 11)
    ]
    ...
    config = BatchConfig(
        max_concurrent=5,
        mode=ProcessingMode.PARALLEL,
        retry_attempts=3  # Will retry failed items
    )
    ...
    report = await processor.process_batch(
        ideas=ideas,
        process_func=processor_with_failures,
        batch_id='example-with-failures'
    )
...
async def example_queue_mode():
    ...
    ideas = [
        {"id": f"idea-{i}", "text": f"Large batch idea {i}"}
