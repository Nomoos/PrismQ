...
Note: Inspiration and IdeaInspiration (junction table) are defined in Model/Entities/
      but not created here — they will be activated by T/Idea/Inspiration.
...
class SimpleIdeaDatabase:
    ...
    def create_tables(self) -> None:
        ...
        Note: version uses INTEGER with CHECK >= 0 to simulate unsigned integer.
        Note: Inspiration and IdeaInspiration tables are defined in Model/Entities/
              but not created here — they will be activated by T/Idea/Inspiration.
        """
        if not self.conn:
            self.connect()
    ...
    def insert_idea(self, text: str, version: int = 1, created_at: Optional[str] = None) -> int:
    ...
    def update_idea(
        self, idea_id: int, text: Optional[str] = None, version: Optional[int] = None
    ) -> bool:
        ...
        if text is not None and version is not None:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE Idea SET text = ?, version = ? WHERE id = ?", (text, version, idea_id)
            )
        elif text is not None:
            cursor = self.conn.cursor()
            cursor.execute("UPDATE Idea SET text = ? WHERE id = ?", (text, idea_id))
        elif version is not None:
            cursor = self.conn.cursor()
            cursor.execute("UPDATE Idea SET version = ? WHERE id = ?", (version, idea_id))
        else:
            return False
