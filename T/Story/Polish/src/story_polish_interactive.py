#!/usr/bin/env python3
"""Step 20+21 — Story Polish: Combined submit + poll runner.

Continuously:
  1. Submits all STORY_POLISH stories to the active track (GPT / Claude / Manual)
  2. Polls all pending polish batches / manual files and processes completed results

On success, polished title + content are saved as new versions and the story
advances to PUBLISHING.
...
_story_polish_dir = _src_dir.parent           # T/Story/Polish
_t_dir = _story_polish_dir.parent.parent      # T
...
import story_polish_batch_submit as _submit
import story_polish_batch_poll as _poll
...
POLL_INTERVAL_IDLE   = 30.0   # seconds — nothing in the polish pipeline
...
def _print_poll(result: dict, run: int) -> None:
    for track, data in result.items():
        ...
        if error:
            ...
            _ok(f"[{run}] {track}: {processed} stories polished → PUBLISHING")
        elif checked > 0:
...
def main() -> int:
    ...
    _header("PrismQ.T.Story.Polish")
    _info(f"Database:    {db_path}")
    _info(f"Polish mode: {review_mode}")
    ...
    try:
        while True:
            ...
            # 1. Submit stories that are waiting in STORY_POLISH state
            try:
                submit_result = _submit.run(conn)
                submitted = submit_result.get("submitted", 0)
                if submitted > 0:
                    mode = submit_result.get("mode", review_mode)
                    _ok(f"[{run}] Submitted {submitted} stories for polish ({mode})")
                    if "openai_batch_id" in submit_result:
                        ...
                        _info(f"  Polish files written to: {submit_result['manual_dir']}")
            except Exception as exc:
                _err(f"[{run}] Submit error: {exc}")

            # 2. Poll all pending tracks (GPT batch, manual)
            try:
            ...
            if pending == 0:
                _info(f"No stories in polish pipeline. Waiting {wait:.0f}s before next check...")
            else:
