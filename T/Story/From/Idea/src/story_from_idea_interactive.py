        ...
        while True:
            ...
            try:
                ...
                if oldest_idea is None:
                    ...
                    print_info(
                        f"Waiting {format_wait_time(wait_interval)} before checking for new ideas..."
                    )
                ...
                if preview:
                    ...
                    if result is None:
                        ...
                        for i, story in enumerate(result.stories):
                            print(
                                f"  Story {i+1}: ID {story.id}, State: {story.state.value if hasattr(story.state, 'value') else story.state}"
                            )

                        if logger:
                            for story in result.stories:
                                logger.info(
                                    f"Created Story ID {story.id} for Idea ID {story.idea_id}"
                                )
