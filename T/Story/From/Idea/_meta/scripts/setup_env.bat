@echo off
REM setup_env.bat - Setup Python Virtual Environment for PrismQ.T.Story.From.Idea
REM This script creates and activates a virtual environment for the module
REM
REM Virtual environment location: T\Story\From\Idea\.venv
REM Dependencies: T\Story\From\Idea\requirements.txt (if exists)
REM
REM This script is designed to be called from other batch scripts in this directory
...
REM Check if requirements need to be installed (only if requirements.txt exists)
set NEEDS_INSTALL=0
...
if not exist "%ENV_FILE%" (
    ...
    echo # PrismQ.T.Story.From.Idea Environment Configuration > "%ENV_FILE%"
