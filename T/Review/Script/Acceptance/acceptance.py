...
def check_script_acceptance(
    ...
    overall_score = int(
        (completeness_score * 0.35) + (coherence_score * 0.35) + (alignment_score * 0.30)
    )
...
def _evaluate_coherence(script_text: str) -> int:
    ...
    connection_count = sum(1 for word in connecting_words if f" {word} " in f" {script_lower} ")
...
def _evaluate_alignment(script_text: str, title: str) -> int:
    ...
    if len(title_words) >= 2:
        ...
        title_bigrams = [
            f"{title_words[i]} {title_words[i+1]}" for i in range(len(title_words) - 1)
        ]
