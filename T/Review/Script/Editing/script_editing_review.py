...
class ScriptEditingChecker:
    ...
    def __init__(self, pass_threshold: int = 85):
        ...
        self.weak_transitions = {
            "and then", "and so", "so anyway", "also", "plus",
            "in addition", "moreover", "furthermore"
        }
        
        # Common redundant pairs
        self.redundant_pairs = [
            "absolutely essential", "advance warning", "basic fundamentals",
            "close proximity", "completely finished", "end result",
            "exact same", "final outcome", "free gift", "future plans",
            "past history", "personal opinion", "prior experience",
            "sudden crisis", "true facts", "unexpected surprise",
            "very unique", "added bonus", "actual fact",
        ]
    ...
    def _generate_feedback(self, review: EditingReview) -> None:
        ...
        # Generate summary
        if review.passes:
            review.summary = f"Script passes editing review with {total_issues} minor issue(s) detected."
        else:
        ...
        if review.critical_count > 0:
            review.primary_concerns.append(f"{review.critical_count} critical clarity issue(s) must be fixed")
...
def get_editing_feedback(review: EditingReview) -> Dict[str, Any]:
    ...
    return {
        ...
        'next_action': 'Proceed to Stage 19 (Title Readability)' if review.passes else 'Return to Script Refinement (Stage 11)'
    }
