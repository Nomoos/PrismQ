#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Review.Content.Editing

This script runs continuously, processing Story objects that need editing review.
...
Usage:
    python editing_workflow.py           # Run continuously with DB save
    python editing_workflow.py --preview # Preview mode (no DB save)
...
# T/Review/Script/Editing/src/ -> T/Review/Script/Editing/ -> T/Review/Script/ -> T/Review/ -> T/ -> repo root
REPO_ROOT = SCRIPT_DIR.parent.parent.parent.parent.parent

sys.path.insert(0, str(REPO_ROOT))

INPUT_STATE = "PrismQ.T.Review.Content.Editing"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Editing Review Workflow for PrismQ"
    )
    ...
    print_header("PrismQ.T.Review.Content.Editing - CONTINUOUS MODE")
    ...
    try:
        while True:
            ...
            print_success(f"Found {pending_count} stories ready for editing review")

            result = process_review_content_editing(connection=conn)
            ...
            if result.accepted:
                print_success(
                    f"Story {result.story.id}: ACCEPTED editing review"
                )
                print_info(f"  Next state: {result.new_state}")
                total_accepted += 1
            else:
                print_warning(
                    f"Story {result.story.id}: REJECTED editing review"
                )
                print_info(f"  Next state: {result.new_state}")
                total_rejected += 1
