...
class TestGrammarReviewProcessing:
    ...
    def _create_test_story_with_script(
        ...
        if created_at:
            ...
            cursor.execute(
                "INSERT INTO Story (state) VALUES (?)",
                (state,)
            )
...
Nature welcomed the night."""
        ...
        cursor = db_connection.execute(
            "SELECT state FROM Story WHERE id = ?",
            (story_id,)
        )
        row = cursor.fetchone()
...
class TestProcessAllPending:
    ...
    def test_process_all_pending(self, db_connection, service):
        ...
        story_ids = [
            self._create_test_story_with_script(
                db_connection, f"Story {i} text."
            )
            for i in range(3)
        ]
    ...
    def test_get_processing_summary(self, db_connection, service):
        ...
        # Good grammar
        self._create_test_story_with_script(
            db_connection, "Perfect grammar here."
        )
        # Bad grammar
        self._create_test_story_with_script(
            db_connection, "He were happy. I recieved gifts."
        )
