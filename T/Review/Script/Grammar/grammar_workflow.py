#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Review.Content.Grammar

This script runs continuously, processing Story objects that need grammar review.
...
Usage:
    python grammar_workflow.py           # Run continuously with DB save
    python grammar_workflow.py --preview # Preview mode (no DB save)
...
# T/Review/Script/Grammar/ -> T/Review/Script/ -> T/Review/ -> T/ -> repo root
...
try:
    from T.Review.Script.Grammar import ScriptGrammarReviewService, INPUT_STATE
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Grammar Review Workflow for PrismQ"
    )
    ...
    try:
        while True:
            ...
            print_success(f"Found {pending_count} stories ready for grammar review")
            ...
            if total_processed % 10 == 0:
                print_info(
                    f"Progress: {total_passed} passed, {total_failed} failed"
                )
        ...
        print()
        print_info(
            f"Session complete: {total_passed} passed, {total_failed} failed"
        )
