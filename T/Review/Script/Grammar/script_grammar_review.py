...
class ScriptGrammarChecker:
    ...
    def __init__(self, pass_threshold: int = 85):
        ...
        self.subject_verb_errors = [
            (r'\b(I|you|we|they)\s+was\b', 'were', 'Subject-verb agreement: plural subjects take "were"'),
            (r'\b(he|she|it)\s+were\b', 'was', 'Subject-verb agreement: singular subjects take "was"'),
            (r'\b(I|you|we|they)\s+has\b', 'have', 'Subject-verb agreement: plural subjects take "have"'),
            (r'\b(he|she|it)\s+have\b', 'has', 'Subject-verb agreement: singular subjects take "has"'),
            (r'\b(I|you|we|they)\s+is\b', 'are', 'Subject-verb agreement: plural subjects take "are"'),
        ...
        self.tense_patterns = {
            ...
            'present': [r'\b(walk|walks|run|runs|say|says|go|goes|come|comes|see|sees|do|does|is|are)\b'],
        }
    ...
    def _check_punctuation(self, line: str, line_num: int, review: GrammarReview) -> None:
        ...
        if len(stripped) > 10 and not re.search(r'[.!?:]$', stripped):
            ...
            if re.search(r'\b(I|you|he|she|it|we|they|the|a|an)\b.*\b(is|are|was|were|has|have|do|does|did)\b', stripped, re.IGNORECASE):
                issue = GrammarIssue(
    ...
    def _check_tense_consistency(self, script_text: str, lines: List[str], review: GrammarReview) -> None:
    ...
    def _generate_feedback(self, review: GrammarReview) -> None:
        ...
        # Generate summary
        if review.passes:
            review.summary = f"Script passes grammar review with {total_issues} minor issue(s) detected."
        else:
        ...
        if review.critical_count > 0:
            review.primary_concerns.append(f"{review.critical_count} critical grammar error(s) must be fixed")
        
        if review.high_count > 0:
            review.primary_concerns.append(f"{review.high_count} high-priority issue(s) should be addressed")
...
def get_grammar_feedback(review: GrammarReview) -> Dict[str, Any]:
    ...
    return {
        ...
        'next_action': 'Proceed to Stage 15 (Tone Review)' if review.passes else 'Return to Script Refinement (Stage 11)'
    }
