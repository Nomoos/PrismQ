...
OUTPUT_STATE_FAIL = StateNames.SCRIPT_FROM_TITLE_REVIEW_SCRIPT  # PrismQ.T.Script.From.Title.Review.Script
...
class ScriptGrammarReviewService:
    ...
    def process_oldest_story(self) -> GrammarReviewResult:
        ...
        try:
            ...
            review = Review(
                text=review_text,
                score=grammar_review.overall_score
            )
    ...
    def _format_review_text(self, grammar_review, feedback: dict) -> str:
        ...
        if grammar_review.issues:
            ...
            for issue in grammar_review.issues[:10]:  # Limit to first 10 issues
                lines.append(f"  Line {issue.line_number} [{issue.severity.value}]: {issue.explanation}")
    ...
    def get_processing_summary(
        self,
        results: List[GrammarReviewResult]
    ) -> dict:
