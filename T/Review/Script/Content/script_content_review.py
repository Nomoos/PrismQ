...
class ScriptContentReviewer:
    ...
    def perform_content_review(
        self, script_text: str, script_id: str, script_version: str = "v3"
    ) -> ContentReview:
    ...
    def process_oldest_story(self) -> Optional[ContentReviewResult]:
        ...
        if script is None:
            return ContentReviewResult(
                story_id=story.id, error=f"Script not found for story {story.id}"
            )

        # Perform content review on script
        content_review = self.perform_content_review(
            script_text=script.text, script_id=str(script.id), script_version=f"v{script.version}"
        )
