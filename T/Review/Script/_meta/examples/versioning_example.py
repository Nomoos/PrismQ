...
def example_version_tracking():
    ...
    review_v1.improvement_points.append(
        ImprovementPoint(
    ...
    print(f"\n✓ Version 2 stored")
    print(
        f"  Score: {review_v2.overall_score}% (+{review_v2.overall_score - review_v1.overall_score}%)"
    )
    ...
    print(f"\n✓ Version 3 stored")
    print(
        f"  Score: {review_v3.overall_score}% (+{review_v3.overall_score - review_v2.overall_score}%)"
    )
    ...
    for v in all_versions:
        print(
            f"V{v.version_number:<9} {str(v.length_seconds)+'s':<12} {str(v.review_score)+'%':<10} {v.changes_from_previous[:47]:<50}"
        )
    ...
    print(
        f"  Length: {first.length_seconds}s → {latest.length_seconds}s (-{length_reduction}s, -{length_pct:.1f}%)"
    )
