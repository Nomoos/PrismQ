...
class ScriptConsistencyChecker:
    ...
    CONTRADICTION_PATTERNS = [
        (
            r"\b(never|no|not|nobody|nothing)\b.*\b(always|yes|everyone|everything)\b",
            "opposite_statements",
        ),
    ...
    def _track_characters(
        self, line: str, line_num: int, character_mentions: Dict[str, List[int]]
    ) -> None:
    ...
    def _track_locations(
        self, line: str, line_num: int, location_mentions: Dict[str, List[int]]
    ) -> None:
    ...
    def _check_timeline(self, line: str, line_num: int, review: ConsistencyReview) -> None:
    ...
    def _check_character_consistency(
        self, character_mentions: Dict[str, List[int]], review: ConsistencyReview
    ) -> None:
            ...
            for name2 in names[i + 1 :]:
                ...
                if self._are_names_similar(name1, name2):
                    issue = ConsistencyIssue(
                        ...
                        related_locations=[
                            f"Lines {', '.join(map(str, character_mentions[name2][:3]))}"
                        ],
                        confidence=75,
                    )
    ...
    def _check_location_consistency(
        self, location_mentions: Dict[str, List[int]], review: ConsistencyReview
    ) -> None:
    ...
    def _check_contradictions(self, lines: List[str], review: ConsistencyReview) -> None:
    ...
    def _generate_feedback(self, review: ConsistencyReview) -> None:
        ...
        if review.critical_count > 0:
            review.primary_concerns.append(
                f"{review.critical_count} critical consistency error(s) must be fixed"
            )

        if review.high_count > 0:
            review.primary_concerns.append(
                f"{review.high_count} high-priority consistency issue(s) should be addressed"
            )
        ...
        if character_count > 0:
            review.primary_concerns.append(
                f"{character_count} character name inconsistenc{'y' if character_count == 1 else 'ies'}"
            )
        ...
        if location_count > 0:
            review.primary_concerns.append(
                f"{location_count} location inconsistenc{'y' if location_count == 1 else 'ies'}"
            )
