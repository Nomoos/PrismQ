#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Review.Content.Consistency

This script runs continuously, processing Story objects that need consistency review.
...
Usage:
    python consistency_workflow.py           # Run continuously with DB save
    python consistency_workflow.py --preview # Preview mode (no DB save)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Consistency Review Workflow for PrismQ"
    )
    ...
    print_header("PrismQ.T.Review.Content.Consistency - CONTINUOUS MODE")
    ...
    try:
        while True:
            ...
            print_success(f"Found {pending_count} stories ready for consistency review")
            ...
            total_processed += 1
            if result.success and result.passes:
                print_success(
