"""Content Tone Review Service - AI-powered review for PrismQ.T.Review.Content.Tone.
...
class ToneReviewResult:
    """Result of tone review processing."""
...
class ScriptToneReviewService:
    """AI-powered tone review service for PrismQ.T.Review.Content.Tone state.
    ...
    def _ai_review(self, content_text: str, title_text: str) -> Tuple[str, int]:
        """Call Ollama for tone review. Returns (feedback, score)."""
        try:
            import requests as _requests
        except ImportError as exc:
            raise RuntimeError("requests library not available; run: pip install requests") from exc

        template = (_PROMPTS_DIR / "review_tone.txt").read_text(encoding="utf-8")
        ...
        feedback = str(data.get("feedback", "AI tone review completed."))
    ...
    def process_oldest_story(self) -> ToneReviewResult:
        """Process the oldest story in REVIEW_CONTENT_TONE state."""
        ...
        result = ToneReviewResult(success=False, story_id=row["story_id"])
        ...
        try:
            ...
            result.success = True
            logger.info(
                f"Story {row['story_id']}: tone review complete, "
                f"score={score}, passes={result.passes}"
            )

        except Exception as e:
            result.error = f"Tone review failed: {str(e)}"
...
def process_review_content_tone(connection: sqlite3.Connection) -> ToneReviewResult:
    """Process the oldest story in PrismQ.T.Review.Content.Tone state.

    Args:
        connection: SQLite database connection with row_factory = sqlite3.Row

    Returns:
        ToneReviewResult with processing details.
    """
    service = ScriptToneReviewService(connection)
