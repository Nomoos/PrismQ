...
class TestGetStoryWithLowestScriptVersion:
    ...
    def test_returns_none_when_story_has_no_scripts(self, db_connection, story_repository):
        ...
        story = Story(idea_json='{"title": "No Scripts"}', state=STATE_REVIEW_SCRIPT_TONE)
...
class TestEvaluateTone:
    ...
    def test_consistent_negative_tone(self):
        ...
        score, text = evaluate_tone(
            script_text="The dark night was filled with fear and horror. The terrible events were scary and ugly. "
            * 5,
        )
...
class TestProcessReviewScriptTone:
    ...
    def test_script_references_review(self, db_connection, story_repository, script_repository):
        ...
        initial_script = Script(
            story_id=story.id, version=0, text="Initial script content about dark horror and fear."
        )
    ...
    def test_updates_story_state(self, db_connection, story_repository, script_repository):
        ...
        # Create a script for the story
        script = Script(story_id=story.id, version=0, text="Test script content about dark horror.")
        script_repository.insert(script)
    ...
    def test_not_accepted_transitions_to_rewrite(
        ...
        script = Script(story_id=story.id, version=0, text="Too short.")
        script_repository.insert(script)
...
class TestGetScriptForStory:
    ...
    def test_returns_script_by_story_script_id(
        self, db_connection, story_repository, script_repository
    ):
        ...
        # Create script
        script = Script(story_id=story.id, version=0, text="Test script content")
    ...
    def test_returns_latest_version_if_no_script_id(
        self, db_connection, story_repository, script_repository
    ):
...
class TestToneReviewWorkflowIntegration:
    ...
    def test_complete_review_flow_accepted(
        ...
        result = process_review_script_tone(db_connection, target_tone="dark horror")
    ...
    def test_complete_review_flow_not_accepted(
        ...
        script = Script(story_id=story.id, version=0, text="Short script.")
        script_repository.insert(script)
