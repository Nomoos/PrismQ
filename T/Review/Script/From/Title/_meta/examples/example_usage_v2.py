...
def main():
    ...
    for imp in improvements[:5]:
        print(f"  ✓ {imp.category}: {imp.v1_score}% → {imp.v2_score}% (+{imp.delta}%) - {imp.feedback}")
    print()
    
    if regressions:
        print(f"Regressions: {len(regressions)}")
        for reg in regressions:
            print(f"  ✗ {reg.category}: {reg.v1_score}% → {reg.v2_score}% ({reg.delta}%) - {reg.feedback}")
        print()
    ...
    print(f"Overall Score Change: {v1_review.overall_score}% → {v2_review.overall_score}% ({score_change:+d}%)")
