...
class TestReview:
    ...
    def test_review_from_dict(self):
        ...
        data = {"text": "Excellent", "score": 95, "id": 5, "created_at": "2024-01-15T10:30:00"}
...
class TestStoryReview:
    ...
    def test_create_story_review(self):
        ...
        story_review = StoryReview(
            story_id=1, review_id=5, version=2, review_type=ReviewType.GRAMMAR
        )
    ...
    def test_version_zero_allowed(self):
        ...
        story_review = StoryReview(
            story_id=1, review_id=1, version=0, review_type=ReviewType.GRAMMAR
        )
        assert story_review.version == 0

    def test_review_type_from_string(self):
        """Test review_type can be created from string."""
        story_review = StoryReview(story_id=1, review_id=1, version=1, review_type="tone")
    ...
    def test_all_review_types(self):
        ...
        for review_type in types:
            story_review = StoryReview(story_id=1, review_id=1, version=1, review_type=review_type)
            assert story_review.review_type.value == review_type

    def test_to_dict(self):
        """Test conversion to dictionary."""
        story_review = StoryReview(
            story_id=1, review_id=5, version=2, review_type=ReviewType.CONTENT, id=10
        )
