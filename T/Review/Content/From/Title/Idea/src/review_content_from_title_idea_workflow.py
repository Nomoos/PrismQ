#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Review.Content.From.Title.Idea

This script runs continuously, processing Story objects that need content review
with both title and idea context.
...
Usage:
    python review_content_from_title_idea_workflow.py           # Run continuously with DB save
    python review_content_from_title_idea_workflow.py --preview # Preview mode (no DB save)
...
try:
    from review_content_from_title_idea_service import ReviewContentFromTitleIdeaService
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Content Review (with Title and Idea context) Workflow for PrismQ",
...
Examples:
  ...
  python review_content_from_title_idea_workflow.py --preview # Preview mode (no DB save)
    ...
    # Setup logging
    if args.debug:
        log_filename = f"review_content_idea_workflow_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        logger = logging.getLogger("PrismQ.Review.Content.From.Title.Idea")
        logger.info("Workflow started")
        print_info(f"Logging to: {log_path}")

    # Print header
    print_header("PrismQ.T.Review.Content.From.Title.Idea - CONTINUOUS MODE")
    ...
    # Initialize service
    service = ReviewContentFromTitleIdeaService(conn, preview_mode=args.preview)
    ...
    try:
        while True:
            ...
            if run_count == 1:
                print_success(f"Found {pending_count} stories ready for content review (with title and idea context)")
            else:
            ...
            if result.success:
                total_processed += 1
                if result.accepted:
                    print_success(f"Story {result.story_id}: Content ACCEPTED (score: {result.score:.2f})")
                    print_info(f"  Next state: {result.next_state}")
                    total_accepted += 1
                else:
                    print_warning(f"Story {result.story_id}: Content REJECTED (score: {result.score:.2f})")
                    print_info(f"  Next state: {result.next_state}")
                    total_rejected += 1
            else:
