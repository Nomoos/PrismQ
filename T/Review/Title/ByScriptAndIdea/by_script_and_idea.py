...
ENGAGEMENT_WORDS = [
    'mystery', 'discover', 'reveal', 'secret', 'hidden', 'shocking', 'amazing',
    'unbelievable', 'terrifying', 'haunting', 'surprising', 'incredible',
...
def analyze_title_idea_alignment(
    ...
    if score >= 80:
        ...
        reasoning = f"Title reflects idea well. {len(matches)}/{len(title_keywords)} keywords present."
    else:
...
def generate_improvement_points(
    ...
    if script_alignment.score < 75:
        if script_alignment.mismatches:
            improvements.append(TitleImprovementPoint(
    ...
    if seo_data['seo_score'] < 70:
        improvements.append(TitleImprovementPoint(
            ...
            priority="medium",
            impact_score=18,
            suggested_fix=f"Consider adding keywords: {', '.join(seo_data['suggested_keywords'])}" if seo_data['suggested_keywords'] else "Add relevant keywords naturally"
        ))
...
def review_title_by_script_and_idea(
    ...
    category_scores = [
        TitleCategoryScore(
            ...
            strengths=[f"Matches: {', '.join(script_alignment.matches)}" if script_alignment.matches else "Title keywords present"],
            weaknesses=[f"Missing: {', '.join(script_alignment.mismatches)}" if script_alignment.mismatches else "Could reference more script elements"]
        ),
        TitleCategoryScore(
            category=TitleReviewCategory.IDEA_ALIGNMENT,
            score=idea_alignment.score,
            reasoning=idea_alignment.reasoning,
            strengths=[f"Aligned keywords: {', '.join(idea_alignment.matches)}" if idea_alignment.matches else "Reflects idea concept"],
            weaknesses=[f"Missing idea elements: {', '.join(idea_alignment.mismatches)}" if idea_alignment.mismatches else "Could strengthen idea connection"]
        ),
        TitleCategoryScore(
            category=TitleReviewCategory.ENGAGEMENT,
            ...
            strengths=["Creates curiosity" if engagement_data['engagement_words'] > 0 else "Clear and direct"],
            weaknesses=["Could use more engaging language" if engagement_data['engagement_words'] == 0 else "Moderate engagement potential"]
        ),
        TitleCategoryScore(
            category=TitleReviewCategory.SEO_OPTIMIZATION,
            score=seo_data['seo_score'],
            reasoning=f"SEO score based on keyword relevance and length optimization",
            strengths=["Good keyword usage" if seo_data['keyword_relevance'] > 70 else "Basic SEO elements"],
            weaknesses=[f"Could add: {', '.join(seo_data['suggested_keywords'])}" if seo_data['suggested_keywords'] else "SEO could be improved"]
        )
    ]
    ...
    review = TitleReview(
        ...
        strengths=[
            ...
            "Good engagement potential" if engagement_data['engagement_score'] >= 70 else "Clear structure"
        ],
        primary_concern=improvement_points[0].description if improvement_points else "No major concerns",
        quick_wins=[imp.title for imp in improvement_points[:3] if imp.priority in ["high", "medium"]]
    )
