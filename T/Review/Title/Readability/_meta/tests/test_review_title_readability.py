...
_module_path = (
    _project_root / "T" / "Review" / "Title" / "Readability" / "src" / "review_title_readability.py"
)
...
class TestGetOldestStoryForReview:
    ...
    def test_returns_oldest_story_in_correct_state(self, story_repository, db_connection):
        ...
        older_story = Story(
            idea_json='{"title": "Older Story"}', state=STATE_REVIEW_TITLE_READABILITY
        )
        older_story = story_repository.insert(older_story)

        # Create a newer story (small delay to ensure different timestamps)
        newer_story = Story(
            idea_json='{"title": "Newer Story"}', state=STATE_REVIEW_TITLE_READABILITY
        )
...
class TestProcessReviewTitleReadability:
    ...
    def test_processes_story_and_returns_result(self, db_connection, story_repository):
        ...
        story = Story(idea_json='{"title": "Test Story"}', state=STATE_REVIEW_TITLE_READABILITY)
        story_repository.insert(story)

        result = process_review_title_readability(
            db_connection, title_text="The Great Mystery Revealed"
        )
    ...
    def test_accepted_transitions_to_story_review(self, db_connection, story_repository):
        ...
        result = process_review_title_readability(
            db_connection, title_text="The Amazing Mystery Adventure"  # Good, engaging title
        )
    ...
    def test_rejected_transitions_to_content_refinement(self, db_connection, story_repository):
        ...
        result = process_review_title_readability(db_connection, title_text="X")  # Too short
...
class TestReviewWorkflowIntegration:
    ...
    def test_complete_acceptance_flow(self, db_connection, story_repository):
        ...
        result = process_review_title_readability(
            db_connection, title_text="The Amazing Mystery Adventure"
        )
    ...
    def test_complete_rejection_flow(self, db_connection, story_repository):
        ...
        # Setup: Create story in correct state
        story = Story(
            idea_json='{"title": "Test", "concept": "A test"}', state=STATE_REVIEW_TITLE_READABILITY
        )
        story = story_repository.insert(story)

        # Process: Execute review with poor title
        result = process_review_title_readability(
            db_connection, title_text="X"  # Very short, should fail
        )
