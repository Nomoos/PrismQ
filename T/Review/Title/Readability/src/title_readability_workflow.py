#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Review.Title.Readability

This script runs continuously, processing Story objects that need title readability review.
...
# T/Review/Title/Readability/src/ -> T/Review/Title/Readability/ -> T/Review/Title/ -> T/Review/ -> T/ -> repo root
REPO_ROOT = SCRIPT_DIR.parent.parent.parent.parent.parent

sys.path.insert(0, str(REPO_ROOT))

INPUT_STATE = "PrismQ.T.Review.Title.Readability"
...
def main():
    ...
    print_header("PrismQ.T.Review.Title.Readability - CONTINUOUS MODE")
    ...
    try:
        while True:
            ...
            print_success(f"Found {pending_count} stories ready for title readability review")

            result = process_review_title_readability(connection=conn)
            ...
            if result.accepted:
                print_success(
                    f"Story {result.story.id}: ACCEPTED title readability review"
                )
                print_info(f"  Next state: {result.new_state}")
                total_accepted += 1
            else:
                print_warning(
                    f"Story {result.story.id}: REJECTED title readability review"
                )
                print_info(f"  Next state: {result.new_state}")
                total_rejected += 1
