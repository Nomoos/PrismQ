...
try:
    from review_title_readability import TitleReadabilityReviewService
...
def main():
    ...
    service = TitleReadabilityReviewService(conn)
    ...
    try:
        while True:
            ...
            if result.passes:
                print_success(
                    f"Story {result.story_id}: ACCEPTED title readability review "
                ...
                print_warning(
                    f"Story {result.story_id}: REJECTED title readability review "
                    f"(score: {result.score:.0f})"
                )
                print_info(f"  Next state: {result.next_state}")
                if result.text:
