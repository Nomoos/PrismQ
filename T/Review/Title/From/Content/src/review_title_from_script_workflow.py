#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Review.Title.From.Content
...
Usage:
    python review_title_from_script_workflow.py           # Run continuously with DB save
    python review_title_from_script_workflow.py --preview # Preview mode (no DB save)
...
REVIEW_TITLE_FROM_SCRIPT_ROOT = SCRIPT_DIR.parent
T_ROOT = REVIEW_TITLE_FROM_SCRIPT_ROOT.parent.parent.parent
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Title Review Workflow for PrismQ",
...
Examples:
  ...
  python review_title_from_script_workflow.py --preview # Preview mode (no DB save)
    ...
    # Setup logging
    if args.debug:
        log_filename = f"review_title_workflow_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    # Print header
    print_header("PrismQ.T.Review.Title.From.Content - CONTINUOUS MODE")
    ...
    # Initialize service
    service = ReviewTitleFromScriptService(conn, preview_mode=args.preview)
    ...
    try:
        while True:
            ...
            if pending_count == 0:
                if run_count == 1:
                    print_info("No stories found with state PrismQ.T.Review.Title.From.Content")
            ...
            if run_count == 1:
                print_success(f"Found {pending_count} stories ready for title review")
            else:
