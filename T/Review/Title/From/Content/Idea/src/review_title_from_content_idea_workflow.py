...
against content and idea context (Stage 4 in the PrismQ workflow).
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Title Review (with Content and Idea context) Workflow for PrismQ",
...
Examples:
  ...
  python review_title_from_content_idea_workflow.py --preview # Preview mode (no DB save)
    ...
    if args.debug:
        log_filename = (
            f"review_title_content_idea_workflow_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    try:
        while True:
            ...
            if run_count == 1:
                print_success(
                    f"Found {pending_count} stories ready for title review "
                    f"(with content and idea context)"
                )
            else:
            ...
            if result.success:
                ...
                if result.accepted:
                    print_success(
                        f"Story {result.story_id}: Title ACCEPTED (score: {result.score})"
                    )
                    print_info(f"  Next state: {result.next_state}")
                    total_accepted += 1
                else:
                    print_warning(
                        f"Story {result.story_id}: Title REJECTED (score: {result.score})"
                    )
                    print_info(f"  Next state: {result.next_state}")
                    total_rejected += 1
            else:
            ...
            if total_processed + total_errors > 0 and (total_processed + total_errors) % 10 == 0:
                print()
                print_info(
                    f"Progress: {total_accepted} accepted, "
        ...
        print()
        print_info(
            f"Session complete: {total_accepted} accepted, "
            f"{total_rejected} rejected, {total_errors} errors"
        )
