...
try:
    from by_script_v2 import review_title_by_script_v2, compare_reviews, get_improvement_summary
    ...
    while True:
        ...
        try:
            ...
            review = review_title_by_script_v2(
                ...
                title_version="v1",
                script_version="v1"
            ) if REVIEW_V2_AVAILABLE else None
            ...
            if not review and REVIEW_AVAILABLE:
                ...
                print(f"  Matches: {', '.join(alignment.matches[:5]) if alignment.matches else 'None'}")
                print(f"  Mismatches: {', '.join(alignment.mismatches[:5]) if alignment.mismatches else 'None'}")
                print(f"  Key Elements: {', '.join(alignment.key_elements[:5]) if alignment.key_elements else 'None'}")
                ...
                if seo['seo_score'] < 70:
                    ...
                    if seo['suggested_keywords']:
                        print_info(f"Suggested keywords: {', '.join(seo['suggested_keywords'][:5])}")
