...
class ReviewTitleFromScriptService:
    ...
    def _simple_review(self, title_text: str, script_text: str) -> Tuple[str, int]:
        ...
        if score >= 80:
            review_text = f"Title aligns well with script. {matches}/{len(title_words)} keywords match."
        elif score >= 60:
            review_text = f"Fair title-script alignment. {matches}/{len(title_words)} keywords found."
        else:
            review_text = f"Title needs improvement. Only {matches}/{len(title_words)} keywords match script."
    ...
    def process_story(self, story: Story) -> ReviewTitleFromScriptResult:
        ...
        if not title:
            return ReviewTitleFromScriptResult(
                success=False,
                story_id=story.id,
                error_message="No title found for story"
            )
        ...
        if not script:
            return ReviewTitleFromScriptResult(
                success=False,
                story_id=story.id,
                error_message="No script found for story"
            )
    ...
    def process_oldest_story(self) -> ReviewTitleFromScriptResult:
        ...
        if not story:
            return ReviewTitleFromScriptResult(
                success=False,
                error_message=f"No stories found in state '{self.CURRENT_STATE}'"
            )
