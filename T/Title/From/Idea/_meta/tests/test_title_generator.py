...
class TestTitleGenerator:
    ...
    def test_generate_from_concept_only(self):
        ...
        assert all(
            "quantum" in v.text.lower() or "computing" in v.text.lower() for v in variants[:1]
        )

    def test_variant_length_constraints(self):
        """Test that variants respect length constraints."""
        config = TitleConfig(min_length=20, max_length=80, num_variants=4)
    ...
    def test_educational_genre_variant(self):
        ...
        assert any("Guide" in text or "Understanding" in text or "How to" in text for text in texts)
    ...
    def test_entertainment_genre_variant(self):
        ...
        texts = [v.text for v in variants]
        assert any("Fascinating" in text or "World" in text or "Secrets" in text for text in texts)
    ...
    def test_variant_to_dict(self):
        ...
        variant = TitleVariant(
            text="Test Title", style="direct", length=10, keywords=["test", "title"], score=0.85
        )
    ...
    def test_custom_config(self):
        ...
        config = TitleConfig(num_variants=4, min_length=30, max_length=60, focus="engagement")

        idea = Idea(
            title="Tech Trends", concept="Latest technology trends", status=IdeaStatus.DRAFT
        )
...
class TestConvenienceFunction:
    ...
    def test_with_custom_config(self):
        ...
        config = TitleConfig(num_variants=4, max_length=70)

        idea = Idea(
            title="Blockchain Technology", concept="Decentralized systems", status=IdeaStatus.DRAFT
        )
...
class TestVariantStyles:
    ...
    def test_question_variant(self):
        ...
        idea = Idea(title="Future of Work", concept="Workplace trends", status=IdeaStatus.DRAFT)
    ...
    def test_howto_variant(self):
        ...
        idea = Idea(title="Data Science", concept="Learning data science", status=IdeaStatus.DRAFT)
    ...
    def test_curiosity_variant(self):
        ...
        idea = Idea(
            title="Neural Networks", concept="Deep learning systems", status=IdeaStatus.DRAFT
        )
        ...
        assert any(
            word in curiosity.text
            for word in ["Hidden", "Secret", "Truth", "Guide", "Complete", "Fascinating"]
        )
    ...
    def test_authoritative_variant(self):
        ...
        assert any(
            word in auth.text
            for word in ["Analysis", "Comprehensive", "Expert", "Essential", "Guide"]
        )

    def test_listicle_variant(self):
        """Test listicle variant generation."""
        idea = Idea(
            title="Productivity Tips", concept="Improving work efficiency", status=IdeaStatus.DRAFT
        )
    ...
    def test_problem_solution_variant(self):
        ...
        idea = Idea(title="Software Bugs", concept="Fixing code issues", status=IdeaStatus.DRAFT)
        ...
        assert any(
            word in problem.text
            for word in ["Solving", "Problems", "Solutions", "Fixing", "Overcoming"]
        )
    ...
    def test_comparison_variant(self):
        ...
        assert any(
            word in comparison.text
            for word in ["vs", "Comparing", "Myths", "Reality", "Old", "New"]
        )

    def test_ultimate_guide_variant(self):
        """Test ultimate guide variant generation."""
        idea = Idea(title="Python Programming", concept="Learning Python", status=IdeaStatus.DRAFT)
        ...
        assert any(
            word in guide.text for word in ["Ultimate", "Complete", "Everything", "Mastering"]
        )

    def test_benefit_variant(self):
        """Test benefit variant generation."""
        idea = Idea(title="Meditation", concept="Mindfulness practices", status=IdeaStatus.DRAFT)
        ...
        assert any(
            word in benefit.text for word in ["Why", "Matters", "Benefits", "Transform", "Unlock"]
        )
...
class TestEdgeCases:
    ...
    def test_very_short_title(self):
        ...
        idea = Idea(title="AI", concept="Artificial Intelligence overview", status=IdeaStatus.DRAFT)
    ...
    def test_title_with_special_characters(self):
        ...
        idea = Idea(
            title="AI & ML: The Future?", concept="AI and ML trends", status=IdeaStatus.DRAFT
        )
    ...
    def test_unicode_title(self):
        ...
        idea = Idea(title="Technology & Innovation", concept="Tech trends", status=IdeaStatus.DRAFT)
