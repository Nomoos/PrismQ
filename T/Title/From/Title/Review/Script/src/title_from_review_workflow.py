#!/usr/bin/env python3
"""Continuous Workflow Runner for PrismQ.T.Title.From.Title.Review.Content

This script runs continuously, processing Story objects that need title improvement
...
Usage:
    python title_from_review_workflow.py           # Run continuously with DB save
    python title_from_review_workflow.py --preview # Preview mode (no DB save)
...
try:
    from title_from_review_service import TitleFromReviewService
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Continuous Title Improvement Workflow for PrismQ",
...
Examples:
  ...
  python title_from_review_workflow.py --preview # Preview mode (no DB save)
    ...
    # Setup logging
    if args.debug:
        log_filename = f"title_from_review_workflow_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    # Print header
    print_header("PrismQ.T.Title.From.Title.Review.Content - CONTINUOUS MODE")
    ...
    # Initialize service
    service = TitleFromReviewService(conn, preview_mode=args.preview)
    ...
    try:
        while True:
            ...
            if run_count == 1:
                print_success(f"Found {pending_count} stories ready for title improvement")
            else:
            ...
            if result.success:
                total_processed += 1
                print_success(
                    f"Story {result.story_id}: Title improved to v{result.new_title_version}"
                )
