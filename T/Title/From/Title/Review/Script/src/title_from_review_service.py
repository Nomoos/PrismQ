...
1. Selecting the next Story in this state (lowest title version first, highest review score first)
2. Loading current title, content, and title review data
3. Generating an improved title using local AI (Ollama qwen3:32b)
4. Saving the new title version
5. Updating the Story state to the next workflow state

State Transitions:
    PrismQ.T.Title.From.Title.Review.Content -> PrismQ.T.Review.Content.From.Title
"""
...
# AI model for title improvement — qwen3:32b for generation quality
_AI_MODEL = os.getenv("PRISMQ_AI_MODEL_TITLE_IMPROVE", "qwen3:32b")
_AI_TEMPERATURE = 0.7
_AI_MAX_TOKENS = 100
_AI_TIMEOUT = 180  # seconds — larger model needs more time
_MAX_CONTENT_PREVIEW_LENGTH = 1500
...
class TitleFromReviewService:
    ...
    Uses local AI (Ollama) to improve titles based on review feedback:
    1. Finds the next story by priority (lowest title version, highest review score)
    2. Loads the current title, content, and review data
    3. Calls local AI to generate an improved title addressing the review feedback
    4. Saves the new title version
    ...
    def _ai_improve_title(
        ...
        """Improve title using local AI based on review feedback.

        Always calls AI — no algorithmic fallback.

        Args:
            title_text: The original title to improve
            content_text: The script content for context
        ...
        template = _load_prompt("title_improvement.txt")
    ...
    def _fetch_next_story(self):
        ...
        cursor = self._conn.execute(
            ...
            SELECT
                ...
                COALESCE(r.text, '')  AS last_title_review_text
            FROM Story s
            INNER JOIN Title t
                ON t.story_id = s.id
    ...
    def process_oldest_story(self) -> TitleImprovementResult:
        ...
        result = TitleImprovementResult(success=False, story_id=row["story_id"])

        try:
            review_text = row["last_title_review_text"]
            review_score = row["last_title_review_score"]

            logger.info(
                f"Story {row['story_id']}: Improving title v{row['title_version']} "
                f"(review score={review_score})"
            )

            # Generate improved title using local AI
            improved_text = self._ai_improve_title(
            ...
            new_version_num = row["title_version"] + 1
            ...
            logger.info(
                f"Story {row['story_id']}: Title improved from "
                f"v{row['title_version']} to v{new_version_num}: \"{improved_text}\""
            )
