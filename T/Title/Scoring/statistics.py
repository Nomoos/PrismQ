...
def calculate_significance(
    variant_a: VariantMetrics, variant_b: VariantMetrics, metric: str = "ctr"
) -> SignificanceResult:
...
def _calculate_ctr_significance(
    variant_a: VariantMetrics, variant_b: VariantMetrics
) -> SignificanceResult:
...
def _calculate_engagement_significance(
    ...
    if variant_a.engagement_score > variant_b.engagement_score:
        ...
        if variant_b.engagement_score > 0:
            improvement = (
                (variant_a.engagement_score - variant_b.engagement_score)
                / variant_b.engagement_score
            ) * 100
        else:
        ...
        if variant_a.engagement_score > 0:
            improvement = (
                (variant_b.engagement_score - variant_a.engagement_score)
                / variant_a.engagement_score
            ) * 100
        else:
            improvement = float("inf")
...
def _calculate_views_significance(
    ...
    chi2 = ((variant_a.views - expected_a) ** 2 / expected_a) + (
...
def compare_all_variants(
    variants: List[VariantMetrics], metric: str = "ctr"
