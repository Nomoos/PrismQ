...
def create_mock_review(script_text: str, iteration: int) -> ScriptReview:
    ...
    review.category_scores = [
        CategoryScore(
            ...
            reasoning="Strong hook, pacing needs work" if iteration == 1 else "Excellent engagement",
    ...
    if iteration == 1:
        ...
        review.primary_concern = "Length optimization for YouTube shorts (currently 145s, target 90s)"
        review.quick_wins = [
...
def example_complete_feedback_loop():
    ...
    while writer.should_continue_iteration():
        ...
        print(f"   Length: {review.current_length_seconds}s (target: {review.optimal_length_seconds}s)")
        ...
        if not writer.should_continue_iteration():
            ...
            if review.overall_score >= writer.target_score_threshold:
                print(f"✓ TARGET SCORE REACHED ({review.overall_score}% >= {writer.target_score_threshold}%)")
            elif writer.current_iteration >= writer.max_iterations:
                print(f"✓ MAX ITERATIONS REACHED ({writer.current_iteration}/{writer.max_iterations})")
            else:
