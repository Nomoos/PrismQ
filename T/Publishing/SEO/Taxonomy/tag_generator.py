...
from .taxonomy_config import DEFAULT_TAXONOMY, TaxonomyConfig
...
class TagGenerator:
    ...
    def generate_tags(
        self, title: str, script: str, base_keywords: Optional[List[str]] = None
    ) -> TagGenerationResult:
        ...
        filtered_tags = {
            tag: score for tag, score in all_tags.items() if score >= self.config.min_relevance
        }
        ...
        sorted_tags = sorted(deduplicated_tags.items(), key=lambda x: x[1], reverse=True)[
    ...
    def _semantic_expansion(
        self, base_tags: Dict[str, float], title: str, script: str
    ) -> Dict[str, float]:
    ...
    def _deduplicate_tags(self, tags: Dict[str, float]) -> Tuple[Dict[str, float], int]:
