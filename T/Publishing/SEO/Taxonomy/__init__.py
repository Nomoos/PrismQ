...
from .tag_generator import TagGenerationResult, TagGenerator, generate_tags
...
def process_taxonomy(
    ...
    tag_result = tag_gen.generate_tags(title=title, script=script, base_keywords=keywords or [])
    ...
    result = {
        ...
        "stats": {
            ...
            "avg_tag_relevance": (
                sum(tag_result.relevance_scores.values()) / len(tag_result.relevance_scores)
                if tag_result.relevance_scores
                else 0
            ),
            "avg_category_confidence": (
                sum(category_result.confidence_scores.values())
                / len(category_result.confidence_scores)
                if category_result.confidence_scores
                else 0
            ),
...
def _calculate_quality_score(
    tag_result: TagGenerationResult, category_result: CategoryClassificationResult
) -> int:
    ...
    if category_result.categories:
        ...
        avg_confidence = sum(category_result.confidence_scores.values()) / len(
            category_result.confidence_scores
        )
