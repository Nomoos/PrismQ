...
class KeywordExtractor:
    ...
    def extract_keywords(
        self, title: str, script: str, method: str = "tfidf"
    ...
    def _extract_frequency(self, tokens: List[str]) -> Dict[str, float]:
        ...
        filtered_tokens = [
            ...
            if (
                ...
                and len(token) <= self.max_keyword_length
                and token.isalnum()
            )
        ]
        ...
        normalized = {word: count / max_freq for word, count in frequency.items()}

        return normalized

    def _extract_hybrid(self, title: str, script: str, tokens: List[str]) -> Dict[str, float]:
    ...
    def _filter_keywords(self, keywords_with_scores: Dict[str, float]) -> Dict[str, float]:
        """Filter and rank keywords.
        ...
        filtered = {
            word: score
            for word, score in keywords_with_scores.items()
            if (len(word) >= self.min_keyword_length and len(word) <= self.max_keyword_length)
        }
    ...
    def _calculate_density(self, keywords: Dict[str, float], total_words: int) -> Dict[str, float]:
    ...
    def suggest_related_keywords(
        self, keywords: List[str], original_text: str, max_suggestions: int = 10
    ) -> List[str]:
            ...
            if token in keywords:
                ...
                for word in context_words:
                    if (
                        ...
                        and len(word) >= self.min_keyword_length
                        and len(word) <= self.max_keyword_length
                        and word.isalnum()
                    ):
                        related_words.add(word)

        # Rank by frequency
        related_freq = Counter([t for t in tokens if t in related_words])

        # Return top suggestions
        top_related = [word for word, count in related_freq.most_common(max_suggestions)]
...
def extract_keywords(
    ...
    """
    extractor = KeywordExtractor(primary_count=primary_count, secondary_count=secondary_count)
