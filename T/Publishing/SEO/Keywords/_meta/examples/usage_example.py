...
def example_basic_usage():
    ...
    print(f"  OG Description ({len(result['og_description'])} chars): {result['og_description'][:80]}...")
...
def example_with_brand():
    ...
    # Without brand
    result_no_brand = process_content_seo(
        title=title,
        script=script
    )
    
    # With brand
    result_with_brand = process_content_seo(
        title=title,
        script=script,
        brand_name="DevMaster"
    )
