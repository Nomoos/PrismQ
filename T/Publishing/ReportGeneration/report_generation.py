...
class ReportGenerator:
    ...
    def _generate_summary(self, report: PublishingReport) -> None:
        ...
        if report.total_reviews > 0:
            achievements.append(
                f"Passed {report.total_reviews} quality review(s)"
            )
        ...
        summary_parts = [
            f"Publishing workflow completed for '{report.title}'."
        ]
        
        if report.total_iterations > 0:
            summary_parts.append(
                f"Content refined through {report.total_iterations} iteration(s)."
            )
        
        if report.workflow_duration:
            summary_parts.append(
                f"Total workflow duration: {report.workflow_duration}."
            )
        
        summary_parts.append(
            "All quality gates passed and content exported successfully."
        )
    ...
    def _format_as_markdown(self, report: PublishingReport) -> str:
        ...
        lines.extend([
            ...
            "---",
            "",
            "*Report generated by PrismQ Content Production Platform*"
        ])
