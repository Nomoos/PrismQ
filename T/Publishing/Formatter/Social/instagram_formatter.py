...
class InstagramFormatter(BaseSocialFormatter):
        ...
        try:
            ...
            if len(full_caption) > self.CHAR_LIMIT:
                ...
                truncated = self._truncate_caption(
                    preview, main_content, key_message, hashtags, self.CHAR_LIMIT
                )
            ...
            result.metadata = self._generate_instagram_metadata(full_caption, hashtags)
    ...
    def _extract_key_message(self, script: str) -> str:
        ...
        for sentence in sentences:
            ...
            if any(
                ...
                for word in ["result", "achieve", "transform", "improve", "%", "x"]
            ):
                return sentence
    ...
    def _extract_instagram_hashtags(self, script: str, num_hashtags: int) -> List[str]:
        ...
        all_hashtags = base_hashtags + [
            tag for tag in instagram_hashtags if tag not in base_hashtags
        ]

        return all_hashtags[:num_hashtags]

    def _truncate_caption(
        self, preview: str, main_content: str, key_message: str, hashtags: List[str], limit: int
    ) -> str:
    ...
    def _generate_instagram_metadata(
        ...
        metadata = SocialMediaMetadata(
            ...
            estimated_engagement_score=self._estimate_instagram_engagement(caption, hashtags),
    ...
    def _estimate_instagram_engagement(self, caption: str, hashtags: List[str]) -> int:
