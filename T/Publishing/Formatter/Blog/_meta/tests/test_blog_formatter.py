...
class TestBlogMetadata:
    ...
    def test_metadata_to_dict(self):
        ...
        metadata = BlogMetadata(excerpt="Test excerpt", reading_time="3 min read", word_count=500)
...
class TestBlogFormattedContent:
    ...
    def test_formatted_content_to_dict(self):
        ...
        metadata = BlogMetadata(reading_time="5 min read")
        content = BlogFormattedContent(
            content_id="test-001", title="Test", formatted_content="Content", metadata=metadata
        )
...
class TestEdgeCases:
    ...
    def test_empty_content(self):
        ...
        result = formatter.format_blog(script="", title="Empty Test", content_id="test-301")
    ...
    def test_very_short_content(self):
        ...
        result = formatter.format_blog(
            script="Innovation.", title="Short Test", content_id="test-302"
        )
    ...
    def test_special_characters(self):
        ...
        result = formatter.format_blog(
            script=script, title="Special Chars", content_id="test-304", format_type="html"
        )
...
class TestScriptLengthVariations:
    ...
    def test_500_word_content(self):
        ...
        formatter = BlogFormatter()
        result = formatter.format_blog(script=script, title="500 Words", content_id="test-401")

        assert result.success is True
        assert 450 <= result.metadata.word_count <= 550
        assert (
            "2 min read" in result.metadata.reading_time
    ...
    def test_1000_word_content(self):
        ...
        formatter = BlogFormatter()
        result = formatter.format_blog(script=script, title="1000 Words", content_id="test-402")

        assert result.success is True
        assert 950 <= result.metadata.word_count <= 1050
        assert (
            "4 min read" in result.metadata.reading_time
    ...
    def test_2000_word_content(self):
        ...
        formatter = BlogFormatter()
        result = formatter.format_blog(script=script, title="2000 Words", content_id="test-403")

        assert result.success is True
        assert 1950 <= result.metadata.word_count <= 2050
        assert (
            "9 min read" in result.metadata.reading_time
            or "8 min read" in result.metadata.reading_time
        )
