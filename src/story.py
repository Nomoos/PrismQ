...
class StoryDatabase:
    ...
    def get_stories_by_state(self, state: str) -> List[Dict[str, Any]]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT * FROM Story WHERE state = ? ORDER BY created_at DESC",
            (state,)
        )
    ...
    def get_stories_by_idea_id(self, idea_id: int) -> List[Dict[str, Any]]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT * FROM Story WHERE idea_id = ? ORDER BY created_at DESC",
            (idea_id,)
        )
    ...
    def get_latest_stories(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT * FROM Story ORDER BY created_at DESC LIMIT ?",
            (limit,)
        )
    ...
    def get_recently_updated_stories(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT * FROM Story ORDER BY updated_at DESC LIMIT ?",
            (limit,)
        )
    ...
    def search_stories_by_state(self, query: str) -> List[Dict[str, Any]]:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT * FROM Story WHERE state LIKE ? ORDER BY created_at DESC",
            (f"%{query}%",)
        )
