...
A modular tool for converting nested git repositories to git subtrees, following SOLID principles with proper separation of concerns.
...
- **Two-Phase Conversion**: 
  1. Convert nested repos to subtrees in module roots
  2. Convert module roots to subtrees in PrismQ root
...
- **git_operations.py**: Git-specific operations (remotes, branches, subtrees)
- **repository_scanner.py**: Repository discovery and classification
- **subtree_manager.py**: High-level subtree operations
...
1. **Single Responsibility Principle (SRP)**: Each class has one reason to change
   ...
   - `SubtreeManager`: Manage subtree operations
   - `SubtreeConverter`: Orchestrate conversion workflow
...
5. Add as subtree to the module root
