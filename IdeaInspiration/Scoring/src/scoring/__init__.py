"""Scoring engine for PrismQ.IdeaInspiration.Scoring."""
...
class ScoringEngine:
    ...
    def score_idea_inspiration(self, idea_inspiration) -> 'ScoreBreakdown':
        """Enrich an IdeaInspiration object with comprehensive scoring.
        ...
        text_scores = self.text_scorer.score_text(text_content, title)
        title_scores = self.text_scorer.score_title_quality(title)
        description_scores = self.text_scorer.score_description_quality(description)
        
        # Get engagement scores if metrics are available
        ...
        if metadata:
            ...
            # Reddit format
            elif 'score' in metadata or 'num_comments' in metadata:
                metrics = {
                    'views': metadata.get('num_views', 0) or 0,
                    'likes': metadata.get('score', 0),
                    'comments': metadata.get('num_comments', 0),
                    'shares': 0
                }
            
            if metrics:
                engagement_score, engagement_details = self.calculate_score(metrics)
        
        # Calculate overall composite score
        # Weight text quality and engagement based on available data
        if engagement_score > 0:
            # Both text and engagement available
            overall_score = (
                text_scores['overall_text_score'] * 0.4 +
                title_scores['title_quality_score'] * 0.2 +
                description_scores['description_quality_score'] * 0.1 +
                engagement_score * 0.3
            )
        else:
            # Text quality only
            overall_score = (
                text_scores['overall_text_score'] * 0.5 +
                title_scores['title_quality_score'] * 0.3 +
                description_scores['description_quality_score'] * 0.2
            )
        
        # Build detailed score breakdown
        score_details = {
            'text_quality': text_scores,
            'title_quality': title_scores,
            'description_quality': description_scores,
        ...
        return ScoreBreakdown(
            ...
            title_score=title_scores['title_quality_score'],
            description_score=description_scores['description_quality_score'],
            text_quality_score=text_scores['overall_text_score'],
            engagement_score=engagement_score,
            readability_score=text_scores['readability_score'],
            sentiment_score=text_scores.get('sentiment_score', 0.0),
            seo_score=0.0,  # Placeholder for future SEO scoring
            tags_score=0.0,  # Placeholder for future tags scoring
    ...
    def score_text_content(self, title: str, description: str, 
        ...
        """Score text content without IdeaInspiration object.
        
        Convenience method for scoring text content directly.
        ...
        Returns:
            Dictionary containing text quality scores
        """
        text_scores = self.text_scorer.score_text(text_content, title)
        title_scores = self.text_scorer.score_title_quality(title)
        description_scores = self.text_scorer.score_description_quality(description)
