"""Configuration management for UserFeedbackSource."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            "USER_FEEDBACK_DATABASE_URL",
            "Database URL for UserFeedbackSource (e.g., sqlite:///user_feedback.s3db)",
            f"sqlite:///{Path(self.working_directory) / 'user_feedback.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "user_feedback.s3db")
        
        # YouTube API configuration (for fetching own channel comments)
        self.youtube_api_key = self._get_or_prompt(
            "YOUTUBE_API_KEY",
            "YouTube API key (for fetching own channel comments)",
            "",
            required=False
        )
        
        # Channel configuration
        self.youtube_channel_id = self._get_or_prompt(
            "USER_FEEDBACK_YOUTUBE_CHANNEL_ID",
            "Your YouTube channel ID (for own channel comments)",
            "",
            required=False
        )
        
        # Max comments to fetch per video
        max_comments = self._get_or_prompt(
            "USER_FEEDBACK_MAX_COMMENTS",
            "Maximum comments to fetch per video",
            "100",
            required=False
        )
        self.max_comments = int(max_comments) if max_comments else 100
        
        # Sentiment analysis threshold
        sentiment_threshold = self._get_or_prompt(
            "USER_FEEDBACK_SENTIMENT_THRESHOLD",
            "Sentiment threshold for filtering (0.0-1.0)",
            "0.0",
            required=False
        )
        self.sentiment_threshold = float(sentiment_threshold) if sentiment_threshold else 0.0
