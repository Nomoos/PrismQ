"""Configuration management for CommentMiningSource."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            "COMMENT_MINING_DATABASE_URL",
            "Database URL for CommentMiningSource",
            f"sqlite:///{Path(self.working_directory) / 'comment_mining.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "comment_mining.s3db")
        
        # Platform API keys
        self.youtube_api_key = self._get_or_prompt(
            "YOUTUBE_API_KEY", "YouTube API key (optional)", "", False)
        
        # Max comments to fetch
        max_comments = self._get_or_prompt(
            "COMMENT_MINING_MAX_COMMENTS", "Maximum comments to fetch", "100", False)
