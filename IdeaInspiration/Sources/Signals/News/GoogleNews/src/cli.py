"""Command-line interface for GoogleNewsSource."""
...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = GoogleNewsPlugin(config)
        ...
        click.echo(f"Region: {config.google_news_region}")
        click.echo(f"Timeframe: {config.google_news_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_google_news(signal['metrics'])
