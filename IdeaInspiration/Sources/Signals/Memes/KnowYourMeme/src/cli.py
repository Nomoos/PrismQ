"""Command-line interface for KnowYourMemeSource."""
...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = KnowYourMemePlugin(config)
        ...
        click.echo(f"Region: {config.know_your_meme_region}")
        click.echo(f"Timeframe: {config.know_your_meme_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_know_your_meme(signal['metrics'])
