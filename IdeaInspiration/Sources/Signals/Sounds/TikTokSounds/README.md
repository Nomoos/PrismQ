...
TikTokSoundsSource collects sound signals from TikTok to identify trending audio and music. This source is part of the PrismQ.IdeaInspiration ecosystem and provides early indicators of viral audio trends for video content strategy.

## Features

- ✅ Collects trending TikTok sounds and audio
- ✅ Tracks sound usage counts and trends
...
cd Sources/Signals/Sounds/TikTokSounds
...
- `DATABASE_PATH` - Path to SQLite database (default: `signals_tiktok_sounds.db`)
- `MAX_RESULTS` - Maximum sounds per scrape (default: 25)
...
# Scrape specific sounds
python -m src.cli scrape --sounds "sound1,sound2,sound3"
```
...
```bash
# Export to CSV
python -m src.cli export sounds.csv --format csv

# Export to JSON
python -m src.cli export sounds.json --format json
```
...
│       └── tik_tok_sounds_plugin.py  # TikTok implementation
├── tests/                          # Test suite
