...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = InstagramAudioTrendsPlugin(config)
        ...
        click.echo(f"Region: {config.instagram_audio_trends_region}")
        click.echo(f"Timeframe: {config.instagram_audio_trends_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_instagram_audio_trends(signal['metrics'])
                
                # Save to database with universal metrics
                success = db.insert_signal(
                    source='instagram_audio_trends',
