"""Command-line interface for GeoLocalTrendsSource."""
...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = GeoLocalTrendsPlugin(config)
        ...
        click.echo(f"Region: {config.geo_local_trends_region}")
        click.echo(f"Timeframe: {config.geo_local_trends_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_geo_local_trends(signal['metrics'])
                
                # Save to database with universal metrics
                success = db.insert_signal(
                    source='geo_local_trends',
