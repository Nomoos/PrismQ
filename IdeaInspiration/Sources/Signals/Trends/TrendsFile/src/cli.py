"""Command-line interface for TrendsFileSource."""
...
from .plugins.trends_file_plugin import TrendsFilePlugin
...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = TrendsFilePlugin(config)
        ...
        click.echo(f"Region: {config.trends_file_region}")
        click.echo(f"Timeframe: {config.trends_file_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_trends_file(signal['metrics'])
