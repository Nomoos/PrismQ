...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_default(
            "DATABASE_URL",
            f"sqlite:///{Path(self.working_directory) / 'social_challenge.db'}"
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "social_challenge.db")
        
        # Google Trends configuration
        self.social_challenge_region = self._get_or_default("GOOGLE_TRENDS_REGION", "US")
        self.social_challenge_language = self._get_or_default("GOOGLE_TRENDS_LANGUAGE", "en-US")
        self.social_challenge_timeframe = self._get_or_default("GOOGLE_TRENDS_TIMEFRAME", "now 7-d")
        
        max_results = self._get_or_default("GOOGLE_TRENDS_MAX_RESULTS", "25")
        self.social_challenge_max_results = int(max_results) if max_results else 25
