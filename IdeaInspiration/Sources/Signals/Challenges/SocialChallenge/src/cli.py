"""Command-line interface for SocialChallengeSource."""
...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = SocialChallengePlugin(config)
        ...
        click.echo(f"Region: {config.social_challenge_region}")
        click.echo(f"Timeframe: {config.social_challenge_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_social_challenge(signal['metrics'])
                
                # Save to database with universal metrics
                success = db.insert_signal(
                    source='social_challenge',
