...
def scrape(env_file, keywords, no_interactive):
    ...
    try:
        ...
        try:
            trends_plugin = InstagramHashtagPlugin(config)
        ...
        click.echo(f"Region: {config.instagram_hashtag_region}")
        click.echo(f"Timeframe: {config.instagram_hashtag_timeframe}")
        if keywords:
        ...
        try:
            ...
            for signal in signals:
                ...
                universal_metrics = UniversalMetrics.from_instagram_hashtag(signal['metrics'])
