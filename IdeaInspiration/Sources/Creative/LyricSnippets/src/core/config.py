"""Configuration management for PrismQ.IdeaInspiration.Sources.Creative.LyricSnippetsSource."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            "DATABASE_URL",
            "Database URL (e.g., sqlite:///lyric_snippets.s3db)",
            f"sqlite:///{Path(self.working_directory) / 'lyric_snippets.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "lyric_snippets.s3db")
        
        # Genius API configuration (for lyric scraping)
        self.genius_api_key = self._get_or_prompt(
            "GENIUS_API_KEY",
            "Genius API key (optional, for API-based lyric scraping)",
            "",
            required=False
        )
        
        genius_max_results = self._get_or_prompt(
            "GENIUS_MAX_RESULTS",
            "Maximum results for Genius API",
            "50",
            required=False
        )
        self.genius_max_results = int(genius_max_results) if genius_max_results else 50
