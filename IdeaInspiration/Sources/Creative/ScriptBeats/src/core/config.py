"""Configuration management for PrismQ.IdeaInspiration.Sources.Creative.ScriptBeatsSource."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            "DATABASE_URL",
            "Database URL (e.g., sqlite:///script_beats.s3db)",
            f"sqlite:///{Path(self.working_directory) / 'script_beats.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "script_beats.s3db")
