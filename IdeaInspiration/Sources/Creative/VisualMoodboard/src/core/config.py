"""Configuration management for PrismQ.IdeaInspiration.Sources.Creative.VisualMoodboardSource."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        database_url = self._get_or_prompt(
            "DATABASE_URL",
            "Database URL (e.g., sqlite:///visual_moodboard.s3db)",
            f"sqlite:///{Path(self.working_directory) / 'visual_moodboard.s3db'}",
            required=False
        )
        ...
        if database_url.startswith("sqlite:///"):
            ...
            self.database_path = str(Path(self.working_directory) / "visual_moodboard.s3db")
        
        # Unsplash API configuration (for visual scraping)
        self.unsplash_access_key = self._get_or_prompt(
            "UNSPLASH_ACCESS_KEY",
            "Unsplash Access Key (optional, for API-based image scraping)",
            "",
            required=False
        )
        
        unsplash_max_results = self._get_or_prompt(
            "UNSPLASH_MAX_RESULTS",
            "Maximum results for Unsplash API",
            "30",
            required=False
        )
        self.unsplash_max_results = int(unsplash_max_results) if unsplash_max_results else 30
