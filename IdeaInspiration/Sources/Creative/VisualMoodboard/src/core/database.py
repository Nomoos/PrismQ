"""Database utilities for VisualMoodboardSource using SQLAlchemy."""
...
class LyricSnippet(Base):
    """SQLAlchemy model for visual resource storage."""
...
def insert_resource(database_url: str, source: str, source_id: str, title: str,
    ...
    with get_session(database_url) as session:
        ...
        existing = session.query(VisualResource).filter_by(
            source=source,
            source_id=source_id
        ).first()
...
def get_all_resources(database_url: str, limit: int = 20, order_by: str = "score") -> List[Dict[str, Any]]:
    ...
    """
    with get_session(database_url) as session:
        order_column = getattr(VisualResource, order_by, VisualResource.score)
        results = session.query(VisualResource).order_by(order_column.desc()).limit(limit).all()
...
def count_resources(database_url: str) -> int:
    ...
    """
    with get_session(database_url) as session:
        return session.query(func.count(VisualResource.id)).scalar()
...
def count_by_source(database_url: str, source: str) -> int:
    ...
    """
    with get_session(database_url) as session:
        return session.query(func.count(VisualResource.id)).filter_by(source=source).scalar()
...
def get_unprocessed_records(database_url: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    """
    with get_session(database_url) as session:
        query = session.query(VisualResource).filter_by(processed=False)
...
def mark_as_processed(database_url: str, record_id: int):
    ...
    """
    with get_session(database_url) as session:
        record = session.query(VisualResource).filter_by(id=record_id).first()
        if record:
...
class Database:
    """Manages database operations for visual resource collection.
