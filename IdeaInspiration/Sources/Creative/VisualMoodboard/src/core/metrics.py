...
class CreativeMetrics:
    ...
    @classmethod
    def from_unsplash(cls, unsplash_data: Dict[str, Any]) -> 'CreativeMetrics':
        """Create CreativeMetrics from Unsplash API data.
        
        Args:
            unsplash_data: Data from Unsplash API
            
        Returns:
            CreativeMetrics instance
        """
        user_data = unsplash_data.get('user', {})
        
        metrics = cls(
            content_type='visual',
            content_format='image',
            platform='unsplash',
            source_url=unsplash_data.get('urls', {}).get('regular'),
            creator=user_data.get('name'),
            work_title=unsplash_data.get('description') or unsplash_data.get('alt_description'),
            license_type='Unsplash License',  # Unsplash uses their own license
            popularity_score=min(unsplash_data.get('likes', 0) / 100, 10.0),  # Normalize
            platform_specific=unsplash_data
        )
        
        # Estimate emotional impact from likes and downloads
        likes = unsplash_data.get('likes', 0)
        downloads = unsplash_data.get('downloads', 0)
        
        if likes > 0:
            metrics.emotional_impact = min(likes / 100, 10.0)
        else:
            metrics.emotional_impact = 5.0
        
        # Visual resources tend to be versatile
        metrics.versatility = 7.0  # Default higher versatility for visuals
        metrics.inspiration_value = 6.0
