"""Configuration management for PrismQ.IdeaInspiration.Sources.Events.CalendarHolidays."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        # Database path
        default_db = str(Path(self.working_directory) / "data" / "calendar_holidays.db")
        self.database_path = os.getenv("CALENDAR_HOLIDAYS_DB_PATH")
        
        if not self.database_path:
            self.database_path = self._prompt_for_value(
                "CALENDAR_HOLIDAYS_DB_PATH",
                "Path to calendar holidays database",
                default_db
            )
            if self.database_path:
                set_key(self.env_file, "CALENDAR_HOLIDAYS_DB_PATH", self.database_path)
                load_dotenv(self.env_file, override=True)
        ...
        self.default_country = os.getenv("CALENDAR_DEFAULT_COUNTRY", "US")
        
        # Default year range
        self.default_year = os.getenv("CALENDAR_DEFAULT_YEAR", "2025")
        
        # Max events to fetch
        self.max_events = int(os.getenv("CALENDAR_MAX_EVENTS", "100"))
