"""Configuration management for PrismQ.IdeaInspiration.Sources.Events.EntertainmentReleases."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        # Database path
        default_db = str(Path(self.working_directory) / "data" / "entertainment_releases.db")
        self.database_path = os.getenv("ENTERTAINMENT_RELEASES_DB_PATH")
        
        if not self.database_path:
            self.database_path = self._prompt_for_value(
                "ENTERTAINMENT_RELEASES_DB_PATH",
                "Path to entertainment releases database",
                default_db
            )
            if self.database_path:
                set_key(self.env_file, "ENTERTAINMENT_RELEASES_DB_PATH", self.database_path)
                load_dotenv(self.env_file, override=True)
        
        # TMDB API key (required for TMDB API)
        self.tmdb_api_key = os.getenv("TMDB_API_KEY", "")
        
        # Default media type
        self.default_media_type = os.getenv("ENTERTAINMENT_DEFAULT_MEDIA_TYPE", "movie")
        
        # Default region
        self.default_region = os.getenv("ENTERTAINMENT_DEFAULT_REGION", "US")
        
        # Max releases to fetch
        self.max_releases = int(os.getenv("ENTERTAINMENT_MAX_RELEASES", "50"))
