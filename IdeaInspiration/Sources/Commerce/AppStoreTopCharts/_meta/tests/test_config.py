"""Tests for App Store Top Charts configuration."""
...
def test_config_defaults(temp_dir):
    ...
    env_backup = {}
    env_vars = ['DATABASE_URL', 'APP_STORE_CATEGORIES', 'APP_STORE_MAX_APPS', 'WORKING_DIRECTORY']
    for var in env_vars:
    ...
    try:
        ...
        assert len(config.app_store_categories) > 0
        assert config.app_store_max_apps == 50
    finally:
...
def test_config_category_parsing(temp_dir):
    ...
    with open(env_path, 'w') as f:
        f.write("APP_STORE_CATEGORIES=games, productivity ,social-networking,entertainment\n")
    
    config = Config(str(env_path), interactive=False)
    assert config.app_store_categories == ["games", "productivity", "social-networking", "entertainment"]
