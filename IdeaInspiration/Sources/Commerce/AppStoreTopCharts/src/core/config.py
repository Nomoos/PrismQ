"""Configuration management for PrismQ.IdeaInspiration.Sources.Commerce.AppStoreTopCharts."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        app_store_country = self._get_or_prompt(
            "APP_STORE_COUNTRY",
            "App Store country code (e.g., us, uk, jp)",
            "us",
            required=False
        )
        self.app_store_country = app_store_country
        
        app_store_categories = self._get_or_prompt(
            "APP_STORE_CATEGORIES",
            "App Store categories to scrape (comma-separated)",
            "games,productivity,social-networking,entertainment",
            required=False
        )
        self.app_store_categories = [cat.strip() for cat in app_store_categories.split(',') if cat.strip()]
        
        app_store_max_apps = self._get_or_prompt(
            "APP_STORE_MAX_APPS",
            "Maximum apps to scrape per category",
            "50",
            required=False
        )
        self.app_store_max_apps = int(app_store_max_apps) if app_store_max_apps else 50
        
        # Google Play configuration
        google_play_country = self._get_or_prompt(
            "GOOGLE_PLAY_COUNTRY",
            "Google Play country code (e.g., us, uk, jp)",
            "us",
            required=False
        )
        self.google_play_country = google_play_country
        
        google_play_language = self._get_or_prompt(
            "GOOGLE_PLAY_LANGUAGE",
            "Google Play language code (e.g., en, es, ja)",
            "en",
            required=False
        )
        self.google_play_language = google_play_language
        
        # Rate limiting
        app_store_delay = self._get_or_prompt(
            "APP_STORE_DELAY_SECONDS",
            "Delay between requests (seconds)",
            "1",
            required=False
        )
        self.app_store_delay_seconds = float(app_store_delay) if app_store_delay else 1.0
