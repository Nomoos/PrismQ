"""Configuration management for PrismQ.IdeaInspiration.Sources.Commerce.AmazonBestsellers."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        amazon_categories = self._get_or_prompt(
            "AMAZON_CATEGORIES",
            "Amazon categories to scrape (comma-separated)",
            "Electronics,Books,Home & Kitchen,Sports & Outdoors",
            required=False
        )
        self.amazon_categories = [cat.strip() for cat in amazon_categories.split(',') if cat.strip()]
        
        amazon_max_products = self._get_or_prompt(
            "AMAZON_MAX_PRODUCTS",
            "Maximum products to scrape per category",
            "50",
            required=False
        )
        self.amazon_max_products = int(amazon_max_products) if amazon_max_products else 50
        
        # Amazon domain configuration
        self.amazon_domain = self._get_or_prompt(
            "AMAZON_DOMAIN",
        ...
        # Rate limiting
        amazon_delay = self._get_or_prompt(
            "AMAZON_DELAY_SECONDS",
            "Delay between requests (seconds)",
            "2",
            required=False
        )
        self.amazon_delay_seconds = float(amazon_delay) if amazon_delay else 2.0
