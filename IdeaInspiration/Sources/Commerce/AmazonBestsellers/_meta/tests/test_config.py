"""Tests for Amazon Bestsellers configuration."""
...
def test_config_defaults(temp_dir):
    ...
    env_backup = {}
    env_vars = ['DATABASE_URL', 'AMAZON_CATEGORIES', 'AMAZON_MAX_PRODUCTS', 'WORKING_DIRECTORY']
    for var in env_vars:
    ...
    try:
        ...
        assert len(config.amazon_categories) > 0
        assert config.amazon_max_products == 50
...
def test_config_category_parsing(temp_dir):
    ...
    with open(env_path, 'w') as f:
        f.write("AMAZON_CATEGORIES=Electronics, Books , Home & Kitchen,Sports\n")
    
    config = Config(str(env_path), interactive=False)
    assert config.amazon_categories == ["Electronics", "Books", "Home & Kitchen", "Sports"]
