"""Configuration management for PrismQ.IdeaInspiration.Sources.Commerce.EtsyTrending."""
...
class Config:
    ...
    def _load_configuration(self):
        ...
        self.etsy_api_key = self._get_or_prompt(
            "ETSY_API_KEY",
            "Etsy API key (optional, for API access)",
            "",
            required=False
        )
        
        # Etsy scraping configuration
        etsy_categories = self._get_or_prompt(
            "ETSY_CATEGORIES",
            "Etsy categories to scrape (comma-separated)",
            "jewelry,home-living,art,craft-supplies",
            required=False
        )
        self.etsy_categories = [cat.strip() for cat in etsy_categories.split(',') if cat.strip()]
        
        etsy_max_listings = self._get_or_prompt(
            "ETSY_MAX_LISTINGS",
            "Maximum listings to scrape per category",
            "50",
            required=False
        )
        self.etsy_max_listings = int(etsy_max_listings) if etsy_max_listings else 50
        
        # Search configuration
        etsy_trending_keywords = self._get_or_prompt(
            "ETSY_TRENDING_KEYWORDS",
            "Trending search keywords (comma-separated)",
            "trending,popular,bestseller",
            required=False
        )
        self.etsy_trending_keywords = [kw.strip() for kw in etsy_trending_keywords.split(',') if kw.strip()]
        
        # Rate limiting
        etsy_delay = self._get_or_prompt(
            "ETSY_DELAY_SECONDS",
            "Delay between requests (seconds)",
            "1",
            required=False
        )
        self.etsy_delay_seconds = float(etsy_delay) if etsy_delay else 1.0
