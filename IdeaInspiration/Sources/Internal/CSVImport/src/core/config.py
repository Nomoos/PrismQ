"""Configuration management for PrismQ.IdeaInspiration.Sources.Internal.CSVImport."""
...
class Config:
    ...
    def _load_config(self):
        ...
        self.database_path = self._get_config_value(
            'CSV_IMPORT_DATABASE_PATH',
            os.path.join(self.working_directory, 'Sources', 'Internal', 'CSVImport', 'csv_import.db'),
            'Path to SQLite database for CSV import data'
        )
        
        # Default CSV file path (optional)
        self.default_csv_path = os.getenv('CSV_IMPORT_DEFAULT_PATH', '')
        
        # CSV parsing options
        self.csv_delimiter = os.getenv('CSV_IMPORT_DELIMITER', ',')
        self.csv_encoding = os.getenv('CSV_IMPORT_ENCODING', 'utf-8')
        
        # Batch import settings
        self.batch_size = int(os.getenv('CSV_IMPORT_BATCH_SIZE', '100'))
        
        # Default values for missing fields
        self.default_priority = os.getenv('CSV_IMPORT_DEFAULT_PRIORITY', 'medium')
        self.default_status = os.getenv('CSV_IMPORT_DEFAULT_STATUS', 'new')
        self.default_category = os.getenv('CSV_IMPORT_DEFAULT_CATEGORY', 'general')
