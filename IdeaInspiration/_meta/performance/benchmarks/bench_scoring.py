...
class TestUniversalContentScore:
    ...
    def test_bench_ucs_full_metrics(self, benchmark, engine, sample_metrics):
        ...
        result = benchmark(calc_ucs)
        assert all(key in result for key in ['universal_content_score', 'engagement_rate', 
                                               'watch_through_rate', 'relative_performance_index'])
...
class TestBatchScoring:
    ...
    def batch_ideas(self):
        ...
        ideas = []
        for i in range(10):
            ideas.append(MockIdeaInspiration(
                title=f"Title {i}",
