...
_module_path = (
    ...
    / "Readability"
    / "src"
    / "review_script_readability_service.py"
)
...
class TestGetStoryForReview:
    ...
    def test_returns_none_when_no_stories_in_correct_state(
        self, story_repository, script_repository
    ):
    ...
    def test_skips_stories_without_script(self, story_repository, script_repository):
        ...
        story1 = Story(
            idea_json='{"title": "Story without script"}', state=STATE_REVIEW_SCRIPT_READABILITY
        )
        story1 = story_repository.insert(story1)

        # Create story with script v1
        story2 = Story(
            idea_json='{"title": "Story with script v1"}', state=STATE_REVIEW_SCRIPT_READABILITY
        )
        story2 = story_repository.insert(story2)
    ...
    def test_returns_none_when_all_stories_have_no_script(
        self, story_repository, script_repository
    ):
        """Should return None when all stories lack scripts (invalid state)."""
        # Create stories without scripts
        story1 = Story(
            idea_json='{"title": "Story 1 without script"}', state=STATE_REVIEW_SCRIPT_READABILITY
        )
        story_repository.insert(story1)

        story2 = Story(
            idea_json='{"title": "Story 2 without script"}', state=STATE_REVIEW_SCRIPT_READABILITY
        )
...
Peace filled the air."""
        ...
        score, text, review = evaluate_script_readability(
            script_text=good_script, script_id="good-script-001"
        )
...
The strengths of the sixth method remained unclear."""
        ...
        score, text, review = evaluate_script_readability(
            script_text=difficult_script, script_id="difficult-script-001"
        )
...
Peace filled the air."""
        ...
        result = process_review_script_readability(db_connection, script_text=good_script)

        if result.accepted:
            assert result.new_state == STATE_STORY_REVIEW

    def test_rejected_review_transitions_to_script_refinement(
        self, db_connection, story_repository, script_repository
    ):
...
Subsequently, the methodology employed in the implementation of the aforementioned functionality was unequivocally quintessential."""
        ...
        result = process_review_script_readability(db_connection, script_text=difficult_script)
...
This is a very long sentence that goes on and on without any natural pauses or breathing points making it extremely difficult for a voiceover artist to deliver smoothly and naturally in one breath."""
    ...
    def test_multiple_stories_processed_by_script_version(
        self, db_connection, story_repository, script_repository
    ):
