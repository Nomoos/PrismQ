...
class TestVersionTracking:
    ...
    def test_version_metadata_tracking(self):
        ...
        script_tracker.add_version(1, {"from_idea": 1, "from_title": 1, "review_status": "pending"})

        script_tracker.add_version(
            2,
            {
                "from_idea": 1,
                "from_title": 2,
...
class TestCompleteWorkflowScenario:
    ...
    def test_full_workflow_from_idea_to_refinement(self):
        ...
        assert helper.stage_validator.transition_to("PrismQ.T.Title.From.Idea")
        title_tracker = helper.start_workflow("Title")
        title_tracker.add_version(1, {"stage": "initial", "from_idea": 1, "variants": 5})

        # Stage 3: Script v1 Generation
        assert helper.stage_validator.transition_to("PrismQ.T.Script.From.Idea.Title")
        script_tracker = helper.start_workflow("Script")
        script_tracker.add_version(
            1, {"stage": "initial", "from_idea": 1, "from_title": 1, "word_count": 1500}
        )
        ...
        assert helper.stage_validator.transition_to("PrismQ.T.Script.From.Script.Review.Title")
        script_tracker.add_version(2, {"stage": "reviewed", "from_title": 2, "word_count": 1600})
        ...
        title_tracker.add_version(3, {"stage": "refined", "status": "approved"})

        # Stage 8: Script v3 (final refinement)
        assert helper.stage_validator.transition_to("PrismQ.T.Review.Script.From.Title")
        script_tracker.add_version(
            3, {"stage": "refined", "from_title": 3, "status": "approved", "word_count": 1550}
        )
...
class TestManualCreationPipeline:
    ...
    def test_manual_creation_pipeline_sequence(self):
        ...
        # Verify all version sequences are valid
        assert_version_sequence(idea_tracker.versions), ("Idea version sequence is invalid")
        assert_version_sequence(title_tracker.versions), ("Title version sequence is invalid")
        assert_version_sequence(script_tracker.versions), ("Script version sequence is invalid")

        # Verify current versions after first improvement cycle
        assert idea_tracker.get_current_version() == 1, "Idea should remain at v1"
        assert title_tracker.get_current_version() == 2, "Title should be at v2 after review cycle"
        assert (
            script_tracker.get_current_version() == 2
        ), "Script should be at v2 after review cycle"

        # Verify versions are properly aligned (within 1 version of each other)
        assert helper.validate_cross_version_alignment(1, 2, 2), "Versions are not properly aligned"
        ...
        assert (
            helper.stage_validator.is_valid_path()
    ...
    def test_manual_creation_pipeline_metadata_tracking(self):
        ...
        idea_tracker.add_version(
            1,
            {"method": "manual", "author": "content_creator", "timestamp": "2025-11-24T00:00:00Z"},
        )

        helper.stage_validator.transition_to("PrismQ.T.Title.From.Idea")
        title_tracker = helper.start_workflow("Title")
        title_tracker.add_version(1, {"from_idea": 1, "generation_method": "ai_assisted"})

        helper.stage_validator.transition_to("PrismQ.T.Script.From.Idea.Title")
        script_tracker = helper.start_workflow("Script")
        script_tracker.add_version(
            1, {"from_idea": 1, "from_title": 1, "generation_method": "ai_assisted"}
        )
        ...
        helper.stage_validator.transition_to("PrismQ.T.Script.From.Script.Review.Title")
        script_tracker.add_version(
            2, {"from_review": True, "improvements": ["better flow", "aligned with improved title"]}
        )
    ...
    def test_manual_creation_pipeline_version_alignment(self):
        ...
        assert idea_tracker.get_current_version() == 1, "Idea should never iterate beyond v1"
