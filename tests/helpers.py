...
class WorkflowStageValidator:
    ...
    VALID_TRANSITIONS = {
        ...
        # Story stages
        "PrismQ.T.Story.Review": ["PrismQ.T.Publishing", "PrismQ.T.Story.Polish"],
...
def create_test_idea(
    ...
    **kwargs,
    ...
    return Idea(
        id="test-id", title=title, concept=concept, version=version, status=status, **kwargs
    )
...
def assert_version_sequence(versions: List[int]) -> None:
    ...
    for i in range(1, len(versions)):
        ...
        assert actual == expected, (
            f"Version sequence broken at index {i}. " f"Expected {expected}, got {actual}"
        )


def create_version_history(
    entity_type: str, num_versions: int = 3, metadata_fn: Optional[callable] = None
) -> VersionTracker:
...
class IntegrationTestHelper:
    ...
    def validate_cross_version_alignment(
        self, idea_version: int, title_version: int, script_version: int
    ) -> bool:
        ...
        idea_aligned = (idea_version == 1) or (
            abs(idea_version - max(title_version, script_version)) <= 1
        )
