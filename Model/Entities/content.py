"""Content model for PrismQ content versioning.

Content stores versioned content text content for each story.
Each content can optionally reference a Review for feedback tracking.
Implements IModel interface following Dependency Inversion Principle.

Relationship Pattern:
    - Story 1:N Content (one story has many content versions)
    - Content N:1 Review (each content version can have one review via FK)

Dependency Inversion:
    Content implements the IModel interface, allowing services to
...
@dataclass
class Content(IModel[int]):
    """Content model for versioned content storage.
    
    Stores content text content with version tracking. Each content
    ...
    Example:
        ...
        ... )
        >>> print(f"Story {content.story_id}, v{content.version}")
        Story 1, v0
    ...
    def get_id(self) -> Optional[int]:
        ...
        Returns:
            Optional[int]: The content ID, or None if not persisted yet.
        """
        return self.id
    ...
    def exists(self) -> bool:
        ...
        Returns:
            bool: True if the content has been persisted (has an ID).
        """
        return self.id is not None
    
    def get_created_at(self) -> Optional[datetime]:
        """Return the creation timestamp of this content.
    ...
    def save(self) -> bool:
        """Persist the content to the database.
        
        Note:
            Actual database operations are handled by ContentRepository.
    ...
    def refresh(self) -> bool:
        """Check if the content can be refreshed from the database.
        
        Note:
            Actual database refresh operations are handled by ContentRepository.
            This method provides interface compliance and returns whether
            a refresh would succeed (i.e., if the entity exists).
            
            Use repository.find_by_id(content.get_id()) to actually reload
            data from the database.
        
        Returns:
            bool: True if the content exists (refresh possible), False otherwise.
        """
        return self.exists()
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Content":
        ...
        Args:
            data: Dictionary containing content data. Required keys:
                story_id, version, text. Optional: id, review_id, created_at.
                
        Returns:
            Content: New Content instance populated with data.
        """
    ...
    def create_next_version(self, new_text: str, review_id: Optional[int] = None) -> "Content":
        """Create a new version of this content with updated text.
        
        This method follows the INSERT+READ only architecture - instead of
        updating the current content, it creates a new version with incremented
        version number.
        
        Args:
            new_text: The new content text for the next version.
            review_id: Optional review reference for the new version.
        
        Returns:
            Content: A new Content instance with version incremented by 1.
            
        Example:
            >>> content_v0 = Content(story_id=1, version=0, text="Original content")
            >>> content_v1 = content_v0.create_next_version("Improved content", review_id=5)
            >>> print(content_v1.version)
            1
        """
        return Content(
            story_id=self.story_id,
    ...
    def get_version_info(self) -> str:
        ...
        Example:
            >>> content = Content(story_id=1, version=2, text="Content")
            >>> print(content.get_version_info())
            v2 (story_id=1)
        """
    ...
    def get_sql_schema(cls) -> str:
        ...
        Returns:
            SQL statement to create the Content table with all
        ...
        CREATE INDEX IF NOT EXISTS idx_content_story_version ON Content(story_id, version);
        """
